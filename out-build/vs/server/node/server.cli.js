...
define(["require", "exports", "fs", "url", "child_process", "http", "vs/base/common/process", "vs/base/common/path", "vs/platform/environment/node/argv", "vs/platform/environment/node/wait", "vs/platform/environment/node/stdin"], function (require, exports, _fs, _url, _cp, _http, process_1, path_1, argv_1, wait_1, stdin_1) {
    ...
    async function main(desc, args) {
        ...
        const options = { ...argv_1.$yl, gitCredential: { type: 'string' }, openExternal: { type: 'boolean' } };
        const isSupported = cliCommand ? isSupportedForCmd : isSupportedForPipe;
        for (const optionId in argv_1.$yl) {
        ...
        const parsedArgs = (0, argv_1.$zl)(args, options, errorReporter);
        const mapFileUri = cliRemoteAuthority ? mapFileToRemoteUri : (uri) => uri;
        const verbose = !!parsedArgs['verbose'];
        if (parsedArgs.help) {
            console.log((0, argv_1.$Bl)(desc.productName, desc.executableName, desc.version, options));
            return;
        }
        if (parsedArgs.version) {
            console.log((0, argv_1.$Cl)(desc.version, desc.commit));
            return;
        }
        if (parsedArgs['locate-shell-integration-path']) {
            ...
            console.log((0, path_1.$0d)(__dirname, '../..', 'workbench', 'contrib', 'terminal', 'browser', 'media', file));
            return;
        }
        if (cliPipe) {
        ...
        parsedArgs['_'] = [];
        if (hasReadStdinArg && (0, stdin_1.$E7b)()) {
            try {
                let stdinFilePath = cliStdInFilePath;
                if (!stdinFilePath) {
                    stdinFilePath = (0, stdin_1.$G7b)();
                    await (0, stdin_1.$H7b)(stdinFilePath, verbose); // throws error if file can not be written
                }
        ...
        if (cliCommand) {
            if (parsedArgs['install-extension'] !== undefined || parsedArgs['uninstall-extension'] !== undefined || parsedArgs['list-extensions']) {
                ...
                const cp = _cp.fork((0, path_1.$9d)(__dirname, '../../../server-main.js'), cmdLine, { stdio: 'inherit' });
            ...
            const ext = (0, path_1.$be)(cliCommand);
            if (ext === '.bat' || ext === '.cmd') {
                ...
                const cliCwd = (0, path_1.$_d)(cliCommand);
            ...
            if (parsedArgs['wait']) {
                ...
                waitMarkerFilePath = (0, wait_1.$d7b)(verbose);
            }
            sendToPipe({
                type: 'open',
                fileURIs,
    ...
    function pathToURI(input) {
        input = input.trim();
        input = (0, path_1.$0d)(preferredCwd, input);
