...
define(["require", "exports", "vs/editor/common/core/range"], function (require, exports, range_1) {
    ...
    exports.$hgc = void 0;
    const $hgc = (bytes, pattern, options) => {
        let text;
        ...
        if (patternIndecies.length) {
            ...
            for (const { matchStartIndex, matchedText } of patternIndecies) {
                ...
                const fileRange = new range_1.$ks(startLine, matchStartIndex - lineRanges[startLine].start, endLine, matchStartIndex + matchedText.length - lineRanges[endLine].start);
                const previewRange = new range_1.$ks(0, matchStartIndex - lineRanges[startLine].start - offset, endLine - startLine, matchStartIndex + matchedText.length - lineRanges[endLine].start - (endLine === startLine ? offset : 0));
                const match = {
                    ranges: fileRange,
