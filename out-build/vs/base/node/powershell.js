...
define(["require", "exports", "os", "vs/base/common/path", "vs/base/node/pfs"], function (require, exports, os, path, pfs) {
    ...
    */
    ...
    class PossiblePowerShellExe {
        ...
        async exists() {
            if (this.a === undefined) {
                this.a = await pfs.SymlinkSupport.existsFile(this.exePath);
            }
            return this.a;
        }
    }
    ...
    async function findPSCoreWindowsInstallation({ useAlternateBitness = false, findPreview = false } = {}) {
        ...
        const powerShellInstallBaseDir = path.$9d(programFilesPath, 'PowerShell');
        ...
        for (const item of await pfs.Promises.readdir(powerShellInstallBaseDir)) {
            ...
            const exePath = path.$9d(powerShellInstallBaseDir, item, 'pwsh.exe');
    ...
    async function findPSCoreMsix({ findPreview } = {}) {
        ...
        const msixAppDir = path.$9d(process.env.LOCALAPPDATA, 'Microsoft', 'WindowsApps');
        ...
        for (const subdir of await pfs.Promises.readdir(msixAppDir)) {
            if (pwshMsixDirRegex.test(subdir)) {
                const pwshMsixPath = path.$9d(msixAppDir, subdir, 'pwsh.exe');
    ...
    function findPSCoreDotnetGlobalTool() {
        const dotnetGlobalToolExePath = path.$9d(os.homedir(), '.dotnet', 'tools', 'pwsh.exe');
        return new PossiblePowerShellExe(dotnetGlobalToolExePath, '.NET Core PowerShell Global Tool');
    }
    function findWinPS() {
        const winPSPath = path.$9d(process.env.windir, processArch === 1 /* Arch.x86 */ && osArch !== 1 /* Arch.x86 */ ? 'SysNative' : 'System32', 'WindowsPowerShell', 'v1.0', 'powershell.exe');
