 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "vs/nls!vs/editor/common/viewLayout/viewLineRenderer", "vs/base/common/strings", "vs/editor/common/core/stringBuilder", "vs/editor/common/viewLayout/lineDecorations", "vs/editor/common/viewLayout/linePart"], function (require, exports, nls, strings, stringBuilder_1, lineDecorations_1, linePart_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.$WW = exports.$VW = exports.$UW = exports.$TW = exports.ForeignElementType = exports.$SW = exports.$RW = exports.$QW = exports.$PW = exports.RenderWhitespace = void 0;
    ...
    class $QW {
        constructor(useMonospaceOptimizations, canUseHalfwidthRightwardsArrow, lineContent, continuesWithWrappedLine, isBasicASCII, containsRTL, fauxIndentLength, lineTokens, lineDecorations, tabSize, startVisibleColumn, spaceWidth, middotWidth, wsmiddotWidth, stopRenderingLineAfter, renderWhitespace, renderControlCharacters, fontLigatures, selectionsOnLine) {
            ...
            this.lineDecorations = lineDecorations.sort(lineDecorations_1.$MW.compare);
        ...
        equals(other) {
            return (this.useMonospaceOptimizations === other.useMonospaceOptimizations
                ...
                && lineDecorations_1.$MW.equalsArr(this.lineDecorations, other.lineDecorations)
                && this.lineTokens.equals(other.lineTokens)
                && this.c(other.selectionsOnLine));
        }
    }
    exports.$QW = $QW;
    ...
    class $SW {
        ...
        setColumnInfo(column, partIndex, charIndex, horizontalOffset) {
            ...
            this.e[column - 1] = partData;
            this.f[column - 1] = horizontalOffset;
        }
        ...
        g(charOffset) {
            ...
            return this.e[charOffset];
        }
        getDomPosition(column) {
            const partData = this.g(column - 1);
            const partIndex = $SW.c(partData);
            const charIndex = $SW.d(partData);
            return new $RW(partIndex, charIndex);
        }
        getColumn(domPosition, partLength) {
            const charOffset = this.h(domPosition.partIndex, partLength, domPosition.charIndex);
            return charOffset + 1;
        }
        h(partIndex, partLength, charIndex) {
            if (this.length === 0) {
                return 0;
            }
            ...
            const minEntry = this.e[min];
            const maxEntry = this.e[max];
            ...
            const minPartIndex = $SW.c(minEntry);
            const minCharIndex = $SW.d(minEntry);
            const maxPartIndex = $SW.c(maxEntry);
            let maxCharIndex;
        ...
        inflate() {
            ...
            for (let i = 0; i < this.length; i++) {
                const partData = this.e[i];
                const partIndex = $SW.c(partData);
                const charIndex = $SW.d(partData);
                const visibleColumn = this.f[i];
    ...
    function $UW(input, sb) {
        if (input.lineContent.length === 0) {
            if (input.lineDecorations.length > 0) {
                ...
                sb.appendString(`</span>`);
                const characterMapping = new $SW(1, beforeCount + afterCount);
                characterMapping.setColumnInfo(1, beforeCount, 0, 0);
                return new $TW(characterMapping, false, containsForeignElements);
            }
            // completely empty line
            sb.appendString('<span><span></span></span>');
            return new $TW(new $SW(0, 0), false, 0 /* ForeignElementType.None */);
        }
    ...
    function $WW(input) {
        ...
        const out = $UW(input, sb);
        return new $VW(out.characterMapping, sb.build(), out.containsRTL, out.containsForeignElements);
    }
    exports.$WW = $WW;
    ...
    function transformAndRemoveOverflowing(lineContent, lineContainsRTL, tokens, fauxIndentLength, len) {
        ...
        if (fauxIndentLength > 0) {
            result[resultLen++] = new linePart_1.$LW(fauxIndentLength, '', 0, false);
        }
        ...
        for (let tokenIndex = 0, tokensLen = tokens.getCount(); tokenIndex < tokensLen; tokenIndex++) {
            ...
            if (endIndex >= len) {
                const tokenContainsRTL = (lineContainsRTL ? strings.$1e(lineContent.substring(startOffset, len)) : false);
                result[resultLen++] = new linePart_1.$LW(len, type, 0, tokenContainsRTL);
                break;
            }
            const tokenContainsRTL = (lineContainsRTL ? strings.$1e(lineContent.substring(startOffset, endIndex)) : false);
            result[resultLen++] = new linePart_1.$LW(endIndex, type, 0, tokenContainsRTL);
                ...
                if (lastTokenEndIndex + 50 /* Constants.LongToken */ < tokenEndIndex) {
                    ...
                    for (let j = lastTokenEndIndex; j < tokenEndIndex; j++) {
                        ...
                        if (lastSpaceOffset !== -1 && j - currTokenStart >= 50 /* Constants.LongToken */) {
                            ...
                            result[resultLen++] = new linePart_1.$LW(lastSpaceOffset + 1, tokenType, tokenMetadata, tokenContainsRTL);
                            currTokenStart = lastSpaceOffset + 1;
                            lastSpaceOffset = -1;
                        }
                    }
                    if (currTokenStart !== tokenEndIndex) {
                        result[resultLen++] = new linePart_1.$LW(tokenEndIndex, tokenType, tokenMetadata, tokenContainsRTL);
                    }
                }
                else {
            ...
            for (let i = 0, len = tokens.length; i < len; i++) {
                ...
                if (diff > 50 /* Constants.LongToken */) {
                    ...
                    for (let j = 1; j < piecesCount; j++) {
                        ...
                        result[resultLen++] = new linePart_1.$LW(pieceEndIndex, tokenType, tokenMetadata, tokenContainsRTL);
                    }
                    result[resultLen++] = new linePart_1.$LW(tokenEndIndex, tokenType, tokenMetadata, tokenContainsRTL);
                }
                else {
    ...
    function extractControlCharacters(lineContent, tokens) {
        const result = [];
        let lastLinePart = new linePart_1.$LW(0, '', 0, false);
        let charOffset = 0;
            ...
            for (; charOffset < tokenEndIndex; charOffset++) {
                ...
                if (isControlCharacter(charCode)) {
                    if (charOffset > lastLinePart.endIndex) {
                        ...
                        lastLinePart = new linePart_1.$LW(charOffset, token.type, token.metadata, token.containsRTL);
                        result.push(lastLinePart);
                    }
                    lastLinePart = new linePart_1.$LW(charOffset + 1, 'mtkcontrol', token.metadata, false);
                    result.push(lastLinePart);
                }
            }
            if (charOffset > lastLinePart.endIndex) {
                // emit previous part if it has text
                lastLinePart = new linePart_1.$LW(tokenEndIndex, token.type, token.metadata, token.containsRTL);
                result.push(lastLinePart);
            }
        }
        return result;
    ...
    function _applyRenderWhitespace(input, lineContent, len, tokens) {
        ...
        for (let charIndex = fauxIndentLength; charIndex < len; charIndex++) {
            ...
            if (wasInWhitespace) {
                ...
                if (!isInWhitespace || (!useMonospaceOptimizations && tmpIndent >= tabSize)) {
                    ...
                    if (generateLinePartForEachWhitespace) {
                        ...
                        for (let i = lastEndIndex + 1; i <= charIndex; i++) {
                            result[resultLen++] = new linePart_1.$LW(i, 'mtkw', 1 /* LinePartMetadata.IS_WHITESPACE */, false);
                        }
                    }
                    else {
                        result[resultLen++] = new linePart_1.$LW(charIndex, 'mtkw', 1 /* LinePartMetadata.IS_WHITESPACE */, false);
                    }
                ...
                if (charIndex === tokenEndIndex || (isInWhitespace && charIndex > fauxIndentLength)) {
                    result[resultLen++] = new linePart_1.$LW(charIndex, tokenType, 0, tokenContainsRTL);
                    tmpIndent = tmpIndent % tabSize;
                }
            }
        ...
        if (generateWhitespace) {
            if (generateLinePartForEachWhitespace) {
                ...
                result[resultLen++] = new linePart_1.$LW(len, 'mtkw', 1 /* LinePartMetadata.IS_WHITESPACE */, false);
            }
        }
        else {
            result[resultLen++] = new linePart_1.$LW(len, tokenType, 0, tokenContainsRTL);
        }
        return result;
    }
    /**
    ...
    function _applyInlineDecorations(lineContent, len, tokens, _lineDecorations) {
        _lineDecorations.sort(lineDecorations_1.$MW.compare);
        const lineDecorations = lineDecorations_1.$OW.normalize(lineContent, _lineDecorations);
        ...
        for (let tokenIndex = 0, len = tokens.length; tokenIndex < len; tokenIndex++) {
            ...
            while (lineDecorationIndex < lineDecorationsLen && lineDecorations[lineDecorationIndex].startOffset < tokenEndIndex) {
                ...
                if (lineDecoration.startOffset > lastResultEndIndex) {
                    ...
                    result[resultLen++] = new linePart_1.$LW(lastResultEndIndex, tokenType, tokenMetadata, tokenContainsRTL);
                }
                if (lineDecoration.endOffset + 1 <= tokenEndIndex) {
                    // This line decoration ends before this token ends
                    lastResultEndIndex = lineDecoration.endOffset + 1;
                    result[resultLen++] = new linePart_1.$LW(lastResultEndIndex, tokenType + ' ' + lineDecoration.className, tokenMetadata | lineDecoration.metadata, tokenContainsRTL);
                    lineDecorationIndex++;
                }
                else {
        ...
        if (lineDecorationIndex < lineDecorationsLen && lineDecorations[lineDecorationIndex].startOffset === lastTokenEndIndex) {
            while (lineDecorationIndex < lineDecorationsLen && lineDecorations[lineDecorationIndex].startOffset === lastTokenEndIndex) {
                const lineDecoration = lineDecorations[lineDecorationIndex];
                result[resultLen++] = new linePart_1.$LW(lastResultEndIndex, lineDecoration.className, lineDecoration.metadata, false);
                lineDecorationIndex++;
            }
        }
        return result;
    ...
    function _renderLine(input, sb) {
        ...
        const characterMapping = new $SW(len + 1, parts.length);
        ...
        if (isOverflowing) {
            ...
            sb.appendString(nls.localize(0, null, renderOverflowingCharCount(overflowingCharCount)));
            sb.appendString('</span>');
        }
        sb.appendString('</span>');
        return new $TW(characterMapping, containsRTL, containsForeignElements);
    }
