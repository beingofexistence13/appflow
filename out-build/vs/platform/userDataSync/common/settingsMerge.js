...
define(["require", "exports", "vs/base/common/arrays", "vs/base/common/json", "vs/base/common/jsonEdit", "vs/base/common/jsonFormatter", "vs/base/common/objects", "vs/platform/userDataSync/common/content", "vs/platform/userDataSync/common/userDataSync"], function (require, exports, arrays_1, json_1, jsonEdit_1, jsonFormatter_1, objects, contentUtil, userDataSync_1) {
    ...
    exports.$Zzb = exports.$Yzb = exports.$Xzb = exports.$Wzb = exports.$Vzb = exports.$Uzb = void 0;
    function $Uzb(defaultIgnoredSettings, configurationService, settingsContent) {
        let value = [];
        if (settingsContent) {
        ...
        const added = [], removed = [...(0, userDataSync_1.$vgb)()];
        ...
        return (0, arrays_1.$Kb)([...defaultIgnoredSettings, ...added,].filter(setting => !removed.includes(setting)));
    }
    exports.$Uzb = $Uzb;
    ...
    function getIgnoredSettingsFromContent(settingsContent) {
        const parsed = (0, json_1.$Lm)(settingsContent);
        return parsed ? parsed['settingsSync.ignoredSettings'] || parsed['sync.ignoredSettings'] || [] : [];
    }
    function $Vzb(content, formattingOptions) {
        const source = (0, json_1.$Lm)(content) || {};
        let result = '{}';
        for (const key of Object.keys(source)) {
            const edits = (0, jsonEdit_1.$CS)(result, [key], source[key], formattingOptions);
            result = (0, jsonEdit_1.$FS)(result, edits);
        }
        return result;
    }
    exports.$Vzb = $Vzb;
    function $Wzb(targetContent, sourceContent, ignoredSettings, formattingOptions) {
        if (ignoredSettings.length) {
            const sourceTree = parseSettings(sourceContent);
            const source = (0, json_1.$Lm)(sourceContent) || {};
            const target = (0, json_1.$Lm)(targetContent);
            if (!target) {
            ...
            settingsToAdd.forEach(s => targetContent = $Zzb(s.setting.key, sourceContent, targetContent, formattingOptions));
        }
        return targetContent;
    }
    exports.$Wzb = $Wzb;
    function $Xzb(originalLocalContent, originalRemoteContent, baseContent, ignoredSettings, resolvedConflicts, formattingOptions) {
        const localContentWithoutIgnoredSettings = $Wzb(originalLocalContent, originalRemoteContent, ignoredSettings, formattingOptions);
        ...
        if (remoteForwarded && !localForwarded) {
            return { conflictsSettings: [], localContent: $Wzb(originalRemoteContent, originalLocalContent, ignoredSettings, formattingOptions), remoteContent: null, hasConflicts: false };
        }
        /* local is empty and not synced before */
        if (baseContent === null && $Yzb(originalLocalContent)) {
            const localContent = areSame(originalLocalContent, originalRemoteContent, ignoredSettings) ? null : $Wzb(originalRemoteContent, originalLocalContent, ignoredSettings, formattingOptions);
        ...
        const local = (0, json_1.$Lm)(originalLocalContent);
        const remote = (0, json_1.$Lm)(originalRemoteContent);
        const base = baseContent ? (0, json_1.$Lm)(baseContent) : null;
        ...
        for (const key of baseToLocal.added.values()) {
            ...
            if (baseToRemote.added.has(key)) {
                ...
                remoteContent = $Zzb(key, localContent, remoteContent, formattingOptions);
            }
        }
        ...
        for (const key of baseToRemote.added.values()) {
            ...
            if (baseToLocal.added.has(key)) {
                ...
                localContent = $Zzb(key, remoteContent, localContent, formattingOptions);
            }
        }
    ...
    function areSame(localContent, remoteContent, ignoredSettings) {
        ...
        const local = (0, json_1.$Lm)(localContent);
        const remote = (0, json_1.$Lm)(remoteContent);
        const ignored = ignoredSettings.reduce((set, key) => { set.add(key); return set; }, new Set());
        ...
        for (let index = 0; index < localTree.length; index++) {
            ...
            if (localNode.setting && remoteNode.setting) {
                ...
                if (!objects.$Zm(local[localNode.setting.key], remote[localNode.setting.key])) {
                    return false;
                }
            }
    ...
    function $Zzb(key, sourceContent, targetContent, formattingOptions) {
        const source = (0, json_1.$Lm)(sourceContent);
        const sourceTree = parseSettings(sourceContent);
    ...
    function insertAtLocation(content, key, value, location, tree, formattingOptions) {
        ...
        /* Insert at the end */
        if (location.index === -1) {
            edits = (0, jsonEdit_1.$CS)(content, [key], value, formattingOptions);
        }
        else {
            edits = getEditToInsertAtLocation(content, key, value, location, tree, formattingOptions).map(edit => (0, jsonEdit_1.$DS)(content, edit, formattingOptions)[0]);
        }
        return (0, jsonEdit_1.$FS)(content, edits);
    }
    function getEditToInsertAtLocation(content, key, value, location, tree, formattingOptions) {
        ...
        const eol = (0, jsonFormatter_1.$zS)(formattingOptions, content);
