...
define(["require", "exports", "assert", "vs/base/browser/ui/tree/objectTreeModel", "vs/base/browser/ui/tree/tree", "vs/base/common/async", "vs/base/test/common/utils"], function (require, exports, assert, objectTreeModel_1, tree_1, async_1, utils_1) {
    ...
    suite('ObjectTreeModel', function () {
        ...
        test('ctor', () => {
            const list = [];
            const model = new objectTreeModel_1.$gS('test', toList(list));
            assert(model);
        ...
        test('collapse state is preserved with strict identity', () => {
            const list = [];
            const model = new objectTreeModel_1.$gS('test', toList(list), { collapseByDefault: true });
        ...
        test('sorter', () => {
            ...
            const list = [];
            const model = new objectTreeModel_1.$gS('test', toList(list), { sorter: { compare(a, b) { return compare(a, b); } } });
            const data = [
        ...
        test('issue #95641', async () => {
            ...
            const model = new objectTreeModel_1.$gS('test', toList(list), { filter });
            ...
            await (0, async_1.$Hg)(0); // wait for refilter microtask
