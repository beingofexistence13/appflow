...
define(["require", "exports", "assert", "vs/base/common/uri", "vs/workbench/api/common/extHostDocumentData", "vs/workbench/api/common/extHostTypes", "vs/editor/common/core/range", "vs/base/test/common/mock", "./extHostDocumentData.test.perf-data", "vs/editor/common/core/wordHelper", "vs/base/test/common/utils"], function (require, exports, assert, uri_1, extHostDocumentData_1, extHostTypes_1, range_1, mock_1, perfData, wordHelper_1, utils_1) {
    ...
    suite('ExtHostDocumentData', () => {
        ...
        function assertOffsetAt(line, character, offset) {
            const pos = new extHostTypes_1.$4J(line, character);
            const actual = data.document.offsetAt(pos);
            assert.strictEqual(actual, offset);
        }
        setup(function () {
            data = new extHostDocumentData_1.$5L(undefined, uri_1.URI.file(''), [
                'This is line one',
        ...
        test('save, when disposed', function () {
            let saved;
            const data = new extHostDocumentData_1.$5L(new class extends (0, mock_1.$rT)() {
                $trySaveDocument(uri) {
                    assert.ok(!saved);
                    saved = uri;
        ...
        test('getWordRangeAtPosition', () => {
            ...
            let range = data.document.getWordRangeAtPosition(new extHostTypes_1.$4J(0, 2));
            assert.strictEqual(range.start.line, 0);
            assert.strictEqual(range.start.character, 0);
            assert.strictEqual(range.end.line, 0);
            assert.strictEqual(range.end.character, 4);
            // ignore bad regular expresson /.*/
            assert.throws(() => data.document.getWordRangeAtPosition(new extHostTypes_1.$4J(0, 2), /.*/));
            range = data.document.getWordRangeAtPosition(new extHostTypes_1.$4J(0, 5), /[a-z+]+/);
            assert.strictEqual(range.start.line, 0);
            assert.strictEqual(range.start.character, 5);
            assert.strictEqual(range.end.line, 0);
            assert.strictEqual(range.end.character, 14);
            range = data.document.getWordRangeAtPosition(new extHostTypes_1.$4J(0, 17), /[a-z+]+/);
            assert.strictEqual(range.start.line, 0);
            assert.strictEqual(range.start.character, 15);
            assert.strictEqual(range.end.line, 0);
            assert.strictEqual(range.end.character, 18);
            range = data.document.getWordRangeAtPosition(new extHostTypes_1.$4J(0, 11), /yy/);
        ...
        test('getWordRangeAtPosition doesn\'t quite use the regex as expected, #29102', function () {
            ...
            let range = data.document.getWordRangeAtPosition(new extHostTypes_1.$4J(0, 0), /\/\*.+\*\//);
            assert.strictEqual(range, undefined);
            range = data.document.getWordRangeAtPosition(new extHostTypes_1.$4J(1, 0), /\/\*.+\*\//);
            assert.strictEqual(range.start.line, 1);
            assert.strictEqual(range.start.character, 0);
            assert.strictEqual(range.end.line, 1);
            assert.strictEqual(range.end.character, 14);
            range = data.document.getWordRangeAtPosition(new extHostTypes_1.$4J(3, 0), /("|').*\1/);
            assert.strictEqual(range, undefined);
            range = data.document.getWordRangeAtPosition(new extHostTypes_1.$4J(3, 1), /("|').*\1/);
        ...
        test('getWordRangeAtPosition can freeze the extension host #95319', function () {
            ...
            const config = (0, wordHelper_1.$Yr)({ maxLen: 1000, windowSize: 15, timeBudget: 30 });
            try {
                let range = data.document.getWordRangeAtPosition(new extHostTypes_1.$4J(0, 1177170), regex);
                assert.strictEqual(range, undefined);
                const pos = new extHostTypes_1.$4J(0, 1177170);
        ...
        test('Rename popup sometimes populates with text on the left side omitted #96013', function () {
            ...
            const range = data.document.getWordRangeAtPosition(new extHostTypes_1.$4J(0, 27), regex);
            assert.strictEqual(range.start.line, 0);
            assert.strictEqual(range.end.line, 0);
        ...
        test('Custom snippet $TM_SELECTED_TEXT not show suggestion #108892', function () {
            ...
            const pos = new extHostTypes_1.$4J(0, 55);
    ...
    suite('ExtHostDocumentData updates line mapping', () => {
        ...
        function assertDocumentLineMapping(doc, direction) {
            ...
            for (let offset = 0; offset <= allText.length; offset++) {
                ...
                const position = new extHostTypes_1.$4J(line, character + (previousIsCarriageReturn ? -1 : 0));
        ...
        function testLineMappingDirectionAfterEvents(lines, eol, direction, e) {
            const myDocument = new extHostDocumentData_1.$5L(undefined, uri_1.URI.file(''), lines.slice(0), eol, 1, 'text', false);
        ...
        test('after remove', () => {
            testLineMappingAfterEvents([
                ...
                'it is followed by #3',
                'and finished with the fourth.',
            ], createChangeEvent(new range_1.$ks(1, 3, 1, 6), ''));
        });
        test('after replace', () => {
            testLineMappingAfterEvents([
                'This is line one',
                ...
                'it is followed by #3',
                'and finished with the fourth.',
            ], createChangeEvent(new range_1.$ks(1, 3, 1, 6), 'is could be'));
        });
        ...
        test('after remove line', () => {
            testLineMappingAfterEvents([
                ...
                'it is followed by #3',
                'and finished with the fourth.',
            ], createChangeEvent(new range_1.$ks(1, 3, 2, 6), ''));
        });
        test('after remove two lines', () => {
            testLineMappingAfterEvents([
                ...
                'it is followed by #3',
                'and finished with the fourth.',
            ], createChangeEvent(new range_1.$ks(1, 3, 3, 6), ''));
        });
        test('after deleting entire content', () => {
            testLineMappingAfterEvents([
                ...
                'it is followed by #3',
                'and finished with the fourth.',
            ], createChangeEvent(new range_1.$ks(1, 3, 4, 30), ''));
        });
        test('after replacing entire content', () => {
            testLineMappingAfterEvents([
                ...
                'it is followed by #3',
                'and finished with the fourth.',
            ], createChangeEvent(new range_1.$ks(1, 3, 4, 30), 'some new text\nthat\nspans multiple lines'));
        });
        test('after changing EOL to CRLF', () => {
            testLineMappingAfterEvents([
                ...
                'it is followed by #3',
                'and finished with the fourth.',
            ], createChangeEvent(new range_1.$ks(1, 1, 1, 1), '', '\r\n'));
        });
