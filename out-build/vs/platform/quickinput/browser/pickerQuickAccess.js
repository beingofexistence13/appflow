...
define(["require", "exports", "vs/base/common/async", "vs/base/common/cancellation", "vs/base/common/lifecycle", "vs/base/common/types"], function (require, exports, async_1, cancellation_1, lifecycle_1, types_1) {
    ...
    exports.$sqb = exports.TriggerAction = void 0;
    var TriggerAction;
    ...
    class $sqb extends lifecycle_1.$kc {
        constructor(c, f) {
            super();
            this.c = c;
            this.f = f;
        ...
        provide(picker, token, runOptions) {
            ...
            picker.canAcceptInBackground = !!this.f?.canAcceptInBackground;
            // Disable filtering & sorting, we control the results
            picker.matchOnLabel = picker.matchOnDescription = picker.matchOnDetail = picker.sortByLabel = false;
            // Set initial picks and update on type
            let picksCts = undefined;
            const picksDisposable = disposables.add(new lifecycle_1.$lc());
            const updatePickerItems = async () => {
                const picksDisposables = picksDisposable.value = new lifecycle_1.$jc();
                ...
                const picksFilter = picker.value.substr(this.c.length).trim();
                const providedPicks = this.g(picksFilter, picksDisposables, picksToken, runOptions);
                const applyPicks = (picks, skipEmpty) => {
                    ...
                    if (items.length === 0) {
                        ...
                        if ((picksFilter.length > 0 || picker.hideInput) && this.f?.noResultsPick) {
                            if ((0, types_1.$xf)(this.f.noResultsPick)) {
                                items = [this.f.noResultsPick(picksFilter)];
                            }
                            else {
                                items = [this.f.noResultsPick];
                            }
                        }
                    }
                    picker.items = items;
                    ...
                    await Promise.all([
                        ...
                        (async () => {
                            if (typeof fastAndSlowPicks.mergeDelay === 'number') {
                                await (0, async_1.$Hg)(fastAndSlowPicks.mergeDelay);
