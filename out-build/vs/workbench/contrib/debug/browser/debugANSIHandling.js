...
define(["require", "exports", "vs/base/common/color", "vs/workbench/contrib/terminal/common/terminalColorRegistry"], function (require, exports, color_1, terminalColorRegistry_1) {
    ...
    exports.$9Rb = exports.$8Rb = exports.$7Rb = void 0;
    /**
     * @param text The content to stylize.
     * @returns An {@link HTMLSpanElement} that contains the potentially stylized text.
     */
    function $7Rb(text, linkDetector, themeService, workspaceFolder) {
        ...
        while (currentPos < textLength) {
            ...
            if (text.charCodeAt(currentPos) === 27 && text.charAt(currentPos + 1) === '[') {
                ...
                if (sequenceFound) {
                    ...
                    $8Rb(root, buffer, styleNames, linkDetector, workspaceFolder, customFgColor, customBgColor, customUnderlineColor);
                    buffer = '';
                    /*
        ...
        function set24BitColor(styleCodes, colorType) {
            if (styleCodes.length >= 5 &&
                ...
                const customColor = new color_1.$Ls(styleCodes[2], styleCodes[3], styleCodes[4]);
        ...
        function set8BitColor(styleCodes, colorType) {
            ...
            if (color) {
                ...
                if (colorType === 'underline') {
                    ...
                    const colorName = terminalColorRegistry_1.$nfb[colorNumber];
        ...
        function setBasicColor(styleCode) {
            ...
            if (colorIndex !== undefined && colorType) {
                const colorName = terminalColorRegistry_1.$nfb[colorIndex];
                const color = theme.getColor(colorName);
                if (color) {
                    changeColor(colorType, color.rgba);
                }
     ...
     * @param linkDetector The {@link $2Pb} responsible for generating links from {@param stringContent}.
     ...
     */
    function $8Rb(root, stringContent, cssClasses, linkDetector, workspaceFolder, customTextColor, customBackgroundColor, customUnderlineColor) {
        ...
        if (customTextColor) {
            container.style.color =
                color_1.$Os.Format.CSS.formatRGB(new color_1.$Os(customTextColor));
        }
        if (customBackgroundColor) {
            container.style.backgroundColor =
                color_1.$Os.Format.CSS.formatRGB(new color_1.$Os(customBackgroundColor));
        }
        if (customUnderlineColor) {
            container.style.textDecorationColor =
                color_1.$Os.Format.CSS.formatRGB(new color_1.$Os(customUnderlineColor));
        }
    ...
    function $9Rb(colorNumber) {
        ...
        if (colorNumber >= 16 && colorNumber <= 231) {
            ...
            return new color_1.$Ls(red, green, blue);
        }
        else if (colorNumber >= 232 && colorNumber <= 255) {
            // Converts to a grayscale value
            colorNumber -= 232;
            const colorLevel = Math.round(colorNumber / 23 * 255);
            return new color_1.$Ls(colorLevel, colorLevel, colorLevel);
        }
        else {
            return;
        }
