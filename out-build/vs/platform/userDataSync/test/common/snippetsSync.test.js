...
"Div": {
    ...
    suite('SnippetsSync', () => {
        ...
        test('when snippets does not exist', async () => {
            ...
            const snippetsResource = testClient.instantiationService.get(userDataProfile_1.$Ek).defaultProfile.snippetsHome;
        ...
        test('first time sync when snippets exists - has conflicts', async () => {
            ...
            const local = (0, resources_1.$ig)(environmentService.userDataSyncHome, testObject.resource, userDataSync_1.$Xgb, 'html.json');
        ...
        test('first time sync when snippets exists - has multiple conflicts', async () => {
            ...
            assert.strictEqual(testObject.status, "hasConflicts" /* SyncStatus.HasConflicts */);
            const environmentService = testClient.instantiationService.get(environment_1.$Ih);
            const local1 = (0, resources_1.$ig)(environmentService.userDataSyncHome, testObject.resource, userDataSync_1.$Xgb, 'html.json');
            const local2 = (0, resources_1.$ig)(environmentService.userDataSyncHome, testObject.resource, userDataSync_1.$Xgb, 'typescript.json');
        ...
        test('first time sync when snippets exists - has multiple conflicts and accept one conflict', async () => {
            ...
            assert.strictEqual(testObject.status, "hasConflicts" /* SyncStatus.HasConflicts */);
            const environmentService = testClient.instantiationService.get(environment_1.$Ih);
            const local = (0, resources_1.$ig)(environmentService.userDataSyncHome, testObject.resource, userDataSync_1.$Xgb, 'typescript.json');
            assertPreviews(testObject.conflicts.conflicts, [local]);
        });
        ...
        test('previews are reset after all conflicts resolved', async () => {
            ...
            await testObject.apply(false);
            const fileService = testClient.instantiationService.get(files_1.$6j);
            assert.ok(!await fileService.exists((0, resources_1.$hg)(conflicts[0].previewResource)));
        });
        test('merge when there are multiple snippets and only one snippet is merged', async () => {
            ...
            assertPreviews(preview.resourcePreviews, [
                (0, resources_1.$ig)(environmentService.userDataSyncHome, testObject.resource, userDataSync_1.$Xgb, 'html.json'),
                (0, resources_1.$ig)(environmentService.userDataSyncHome, testObject.resource, userDataSync_1.$Xgb, 'typescript.json'),
            ]);
            assert.deepStrictEqual(testObject.conflicts.conflicts, []);
        ...
        async function updateSnippet(name, content, client, profile) {
            ...
            const snippetsResource = (0, resources_1.$ig)((profile ?? userDataProfilesService.defaultProfile).snippetsHome, name);
            await fileService.writeFile(snippetsResource, buffer_1.$Fd.fromString(content));
        }
        async function removeSnippet(name, client) {
            const fileService = client.instantiationService.get(files_1.$6j);
            const userDataProfilesService = client.instantiationService.get(userDataProfile_1.$Ek);
            const snippetsResource = (0, resources_1.$ig)(userDataProfilesService.defaultProfile.snippetsHome, name);
