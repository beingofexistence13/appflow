...
define(["require", "exports", "assert", "vs/base/common/htmlContent", "vs/base/common/lifecycle", "vs/base/common/uri", "vs/editor/common/core/range", "vs/editor/common/languages/language", "vs/editor/common/model", "vs/editor/common/services/languageService", "vs/editor/test/common/testTextModel", "vs/platform/instantiation/test/common/instantiationServiceMock", "vs/platform/log/common/log", "vs/workbench/contrib/debug/browser/breakpointEditorContribution", "vs/workbench/contrib/debug/browser/breakpointsView", "vs/workbench/contrib/debug/common/debug", "vs/workbench/contrib/debug/common/debugModel", "vs/workbench/contrib/debug/test/browser/callStack.test", "vs/workbench/contrib/debug/test/browser/mockDebugModel", "vs/workbench/contrib/debug/test/common/mockDebug", "vs/workbench/test/common/workbenchTestServices"], function (require, exports, assert, htmlContent_1, lifecycle_1, uri_1, range_1, language_1, model_1, languageService_1, testTextModel_1, instantiationServiceMock_1, log_1, breakpointEditorContribution_1, breakpointsView_1, debug_1, debugModel_1, callStack_test_1, mockDebugModel_1, mockDebug_1, workbenchTestServices_1) {
    ...
    function addBreakpointsAndCheckEvents(model, uri, data) {
        ...
        const toDispose = model.onDidChangeBreakpoints(e => {
            ...
            for (let i = 0; i < data.length; i++) {
                assert.strictEqual(e.added[i] instanceof debugModel_1.$SFb, true);
    ...
    suite('Debug - Breakpoints', () => {
        ...
        test('two files', () => {
            ...
            assert.strictEqual((0, breakpointsView_1.$9Fb)(model, undefined, 9), 44);
            ...
            assert.strictEqual((0, breakpointsView_1.$9Fb)(model, undefined, 9), 110);
            ...
            assert.strictEqual((0, breakpointsView_1.$9Fb)(model, undefined, 9), 66);
            assert.strictEqual(model.getBreakpoints().length, 3);
        });
        test('conditions', () => {
            const modelUri1 = uri_1.URI.file('/myfolder/my file first.js');
        ...
        test('multiple sessions', () => {
            ...
            const session2 = disposables.add((0, callStack_test_1.$vfc)(model));
            const data2 = new Map();
        ...
        test('message and class name', () => {
            ...
            result = (0, breakpointsView_1.$_Fb)(2 /* State.Stopped */, true, functionBreakpoint);
            ...
            result = (0, breakpointsView_1.$_Fb)(2 /* State.Stopped */, true, breakpoints[0]);
        ...
        test('decorations', () => {
            ...
            const textModel = (0, testTextModel_1.$O0b)(['this is line one', 'this is line two', '    this is line three it has whitespace at start', 'this is line four', 'this is line five'].join('\n'), languageId);
            addBreakpointsAndCheckEvents(model, modelUri, [
            ...
            const breakpoints = model.getBreakpoints();
            const instantiationService = new instantiationServiceMock_1.$L0b();
            instantiationService.stub(debug_1.$nH, new mockDebug_1.$ofc());
            instantiationService.stub(language_1.$ct, disposables.add(new languageService_1.$jmb()));
            let decorations = instantiationService.invokeFunction(accessor => (0, breakpointEditorContribution_1.$bGb)(accessor, textModel, breakpoints, 3 /* State.Running */, true, true));
            assert.strictEqual(decorations.length, 3); // last breakpoint filtered out since it has a large line number
            assert.deepStrictEqual(decorations[0].range, new range_1.$ks(1, 1, 1, 2));
            assert.deepStrictEqual(decorations[1].range, new range_1.$ks(2, 4, 2, 5));
            assert.deepStrictEqual(decorations[2].range, new range_1.$ks(3, 5, 3, 6));
            ...
            const expected = new htmlContent_1.$Xj(undefined, { isTrusted: true, supportThemeIcons: true }).appendCodeblock(languageId, 'Expression condition: x > 5');
            assert.deepStrictEqual(decorations[0].options.glyphMarginHoverMessage, expected);
            decorations = instantiationService.invokeFunction(accessor => (0, breakpointEditorContribution_1.$bGb)(accessor, textModel, breakpoints, 3 /* State.Running */, true, false));
            assert.strictEqual(decorations[0].options.overviewRuler, null);
            textModel.dispose();
            instantiationService.dispose();
        });
        test('updates when storage changes', () => {
            const storage1 = disposables.add(new workbenchTestServices_1.$7dc());
            const debugStorage1 = disposables.add(new mockDebug_1.$sfc(storage1));
            const model1 = disposables.add(new debugModel_1.$YFb(debugStorage1, { isDirty: (e) => false }, mockDebugModel_1.$tfc, new log_1.$fj()));
            ...
            const storage2 = disposables.add(new workbenchTestServices_1.$7dc());
            const model2 = disposables.add(new debugModel_1.$YFb(disposables.add(new mockDebug_1.$sfc(storage2)), { isDirty: (e) => false }, mockDebugModel_1.$tfc, new log_1.$fj()));
