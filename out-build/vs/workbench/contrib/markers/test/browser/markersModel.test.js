...
define(["require", "exports", "assert", "vs/base/common/uri", "vs/platform/markers/common/markers", "vs/workbench/contrib/markers/browser/markersModel", "vs/base/common/collections", "vs/base/test/common/utils"], function (require, exports, assert, uri_1, markers_1, markersModel_1, collections_1, utils_1) {
    ...
    class TestMarkersModel extends markersModel_1.$ySb {
        constructor(markers) {
            super();
            const byResource = (0, collections_1.$I)(markers, r => r.resource.toString());
    ...
    suite('MarkersModel Test', () => {
        ...
        test('toString()', () => {
            ...
            marker.code = '1234';
            assert.strictEqual(JSON.stringify({ ...marker, resource: marker.resource.path }, null, '\t'), new markersModel_1.$vSb('1', marker).toString());
            marker = aMarker('a/res2', markers_1.MarkerSeverity.Warning);
            assert.strictEqual(JSON.stringify({ ...marker, resource: marker.resource.path }, null, '\t'), new markersModel_1.$vSb('2', marker).toString());
            marker = aMarker('a/res2', markers_1.MarkerSeverity.Info, 1, 2, 1, 8, 'Info', '');
            assert.strictEqual(JSON.stringify({ ...marker, resource: marker.resource.path }, null, '\t'), new markersModel_1.$vSb('3', marker).toString());
            marker = aMarker('a/res2', markers_1.MarkerSeverity.Hint, 1, 2, 1, 8, 'Ignore message', 'Ignore');
            assert.strictEqual(JSON.stringify({ ...marker, resource: marker.resource.path }, null, '\t'), new markersModel_1.$vSb('4', marker).toString());
            ...
            const testObject = new markersModel_1.$vSb('5', marker, null);
            // hack
            testObject.relatedInformation = marker.relatedInformation.map(r => new markersModel_1.$xSb('6', marker, r));
