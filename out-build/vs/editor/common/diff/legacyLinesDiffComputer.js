...
define(["require", "exports", "vs/base/common/diff/diff", "vs/editor/common/diff/linesDiffComputer", "./rangeMapping", "vs/base/common/strings", "vs/editor/common/core/range", "vs/base/common/assert", "vs/editor/common/core/lineRange"], function (require, exports, diff_1, linesDiffComputer_1, rangeMapping_1, strings, range_1, assert_1, lineRange_1) {
    ...
    exports.$Bs = exports.$As = void 0;
    const MINIMUM_MATCHING_CHARACTER_LENGTH = 3;
    class $As {
        computeDiff(originalLines, modifiedLines, options) {
            const diffComputer = new $Bs(originalLines, modifiedLines, {
            ...
            for (const c of result.changes) {
                ...
                if (c.originalEndLineNumber === 0) {
                    // Insertion
                    originalRange = new lineRange_1.$ts(c.originalStartLineNumber + 1, c.originalStartLineNumber + 1);
                }
                else {
                    originalRange = new lineRange_1.$ts(c.originalStartLineNumber, c.originalEndLineNumber + 1);
                }
                let modifiedRange;
                if (c.modifiedEndLineNumber === 0) {
                    // Deletion
                    modifiedRange = new lineRange_1.$ts(c.modifiedStartLineNumber + 1, c.modifiedStartLineNumber + 1);
                }
                else {
                    modifiedRange = new lineRange_1.$ts(c.modifiedStartLineNumber, c.modifiedEndLineNumber + 1);
                }
                let change = new rangeMapping_1.$ws(originalRange, modifiedRange, c.charChanges?.map(c => new rangeMapping_1.$xs(new range_1.$ks(c.originalStartLineNumber, c.originalStartColumn, c.originalEndLineNumber, c.originalEndColumn), new range_1.$ks(c.modifiedStartLineNumber, c.modifiedStartColumn, c.modifiedEndLineNumber, c.modifiedEndColumn))));
                if (lastChange) {
                    if (lastChange.modified.endLineNumberExclusive === change.modified.startLineNumber
                        || lastChange.original.endLineNumberExclusive === change.original.startLineNumber) {
                        // join touching diffs. Probably moving diffs up/down in the algorithm causes touching diffs.
                        change = new rangeMapping_1.$ws(lastChange.original.join(change.original), lastChange.modified.join(change.modified), lastChange.innerChanges && change.innerChanges ?
            ...
            return new linesDiffComputer_1.$ys(changes, [], result.quitEarly);
        }
    }
    exports.$As = $As;
    function computeDiff(originalSequence, modifiedSequence, continueProcessingPredicate, pretty) {
        const diffAlgo = new diff_1.$qs(originalSequence, modifiedSequence, continueProcessingPredicate);
