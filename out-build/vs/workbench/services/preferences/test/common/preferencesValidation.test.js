...
define(["require", "exports", "assert", "vs/workbench/services/preferences/common/preferencesValidation"], function (require, exports, assert, preferencesValidation_1) {
    ...
    suite('Preferences Validation', () => {
        class Tester {
            constructor(d) {
                this.d = d;
                this.c = (0, preferencesValidation_1.$pE)(d);
            }
            accepts(input) {
                assert.strictEqual(this.c(input), '', `Expected ${JSON.stringify(this.d)} to accept \`${JSON.stringify(input)}\`. Got ${this.c(input)}.`);
            }
            rejects(input) {
                assert.notStrictEqual(this.c(input), '', `Expected ${JSON.stringify(this.d)} to reject \`${JSON.stringify(input)}\`.`);
                return {
                    withMessage: (message) => {
                        const actual = this.c(input);
                        assert.ok(actual);
                        assert(actual.indexOf(message) > -1, `Expected error of ${JSON.stringify(this.d)} on \`${input}\` to contain ${message}. Got ${this.c(input)}.`);
                    }
                };
            }
            validatesNumeric() {
        ...
        test('getInvalidTypeError', () => {
            function testInvalidTypeError(value, type, shouldValidate) {
                ...
                if (shouldValidate) {
                    assert.ok(!(0, preferencesValidation_1.$qE)(value, type), message);
                }
                else {
                    assert.ok((0, preferencesValidation_1.$qE)(value, type), message);
                }
