...
define(["require", "exports", "electron", "vs/base/common/errors"], function (require, exports, electron_1, errors_1) {
    ...
    class ValidatedIpcMain {
        ...
        b(channel, event) {
            if (!channel || !channel.startsWith('vscode:')) {
                (0, errors_1.$Y)(`Refused to handle ipcMain event for channel '${channel}' because the channel is unknown.`);
            ...
            try {
                ...
                (0, errors_1.$Y)(`Refused to handle ipcMain event for channel '${channel}' because of a malformed URL '${url}'.`);
                return false; // unexpected URL
            }
            if (host !== 'vscode-app') {
                (0, errors_1.$Y)(`Refused to handle ipcMain event for channel '${channel}' because of a bad origin of '${host}'.`);
                return false; // unexpected sender
            }
            if (sender.parent !== null) {
                (0, errors_1.$Y)(`Refused to handle ipcMain event for channel '${channel}' because sender of origin '${host}' is not a main frame.`);
     ...
     */
    exports.$US = new ValidatedIpcMain();
});
