 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/base/common/event", "vs/base/common/lifecycle", "vs/base/test/common/utils", "vs/editor/browser/controller/textAreaInput", "vs/editor/browser/controller/textAreaState"], function (require, exports, assert, event_1, lifecycle_1, utils_1, textAreaInput_1, textAreaState_1) {
    "use strict";
        ...
        async function simulateInteraction(recorded) {
            ...
            const host = {
                ...
                getScreenReaderContent: function () {
                    return new textAreaState_1.$8W('', 0, 0, null, undefined);
                },
                deduceModelPosition: function (viewAnchorPosition, deltaOffset, lineFeedCnt) {
                    throw new Error('Function not implemented.');
                }
            };
            const wrapper = disposables.add(new class extends lifecycle_1.$kc {
                constructor() {
                    super();
                    this.a = this.B(new event_1.$fd());
                    this.onKeyDown = this.a.event;
                    this.b = this.B(new event_1.$fd());
                    this.onKeyPress = this.b.event;
                    this.c = this.B(new event_1.$fd());
                    this.onKeyUp = this.c.event;
                    this.g = this.B(new event_1.$fd());
                    this.onCompositionStart = this.g.event;
                    this.h = this.B(new event_1.$fd());
                    this.onCompositionUpdate = this.h.event;
                    this.j = this.B(new event_1.$fd());
                    this.onCompositionEnd = this.j.event;
                    this.m = this.B(new event_1.$fd());
                    this.onBeforeInput = this.m.event;
                    this.n = this.B(new event_1.$fd());
                    this.onInput = this.n.event;
                ...
                _initialize(state) {
                    this.r.value = state.value;
                    this.r.selectionStart = state.selectionStart;
                    this.r.selectionEnd = state.selectionEnd;
                }
                _dispatchRecordedEvent(event) {
                    this.s = event;
                    this.r.value = event.state.value;
                    this.r.selectionStart = event.state.selectionStart;
                    this.r.selectionEnd = event.state.selectionEnd;
                    this.r.selectionDirection = event.state.selectionDirection;
                    if (event.type === 'keydown' || event.type === 'keypress' || event.type === 'keyup') {
                        ...
                        if (event.type === 'keydown') {
                            this.a.fire(mockEvent);
                        }
                        else if (event.type === 'keypress') {
                            this.b.fire(mockEvent);
                        }
                        else {
                            this.c.fire(mockEvent);
                        }
                    }
                        ...
                        if (event.type === 'compositionstart') {
                            this.g.fire(mockEvent);
                        }
                        else if (event.type === 'compositionupdate') {
                            this.h.fire(mockEvent);
                        }
                        else {
                            this.j.fire(mockEvent);
                        }
                    }
                        ...
                        if (event.type === 'beforeinput') {
                            this.m.fire(mockEvent);
                        }
                        else {
                            this.n.fire(mockEvent);
                        }
                    }
                    else {
                ...
                setValue(reason, value) {
                    if (this.s?.type === 'compositionstart') {
                        assert.fail('should not change the state of the textarea in a compositionstart');
                    }
                    this.r.value = value;
                }
                getSelectionStart() {
                    return this.r.selectionDirection === 'backward' ? this.r.selectionEnd : this.r.selectionStart;
                }
                getSelectionEnd() {
                    return this.r.selectionDirection === 'backward' ? this.r.selectionStart : this.r.selectionEnd;
                }
                setSelectionRange(reason, selectionStart, selectionEnd) {
                    if (this.s?.type === 'compositionstart') {
                        assert.fail('should not change the state of the textarea in a compositionstart');
                    }
                    this.r.selectionStart = selectionStart;
                    this.r.selectionEnd = selectionEnd;
                    this.r.selectionDirection = (selectionStart !== selectionEnd ? 'forward' : 'none');
                }
            ...
            const input = disposables.add(new textAreaInput_1.$_W(host, wrapper, recorded.env.OS, recorded.env.browser));
