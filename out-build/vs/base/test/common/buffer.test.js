...
define(["require", "exports", "assert", "vs/base/common/async", "vs/base/common/buffer", "vs/base/common/stream", "vs/base/test/common/utils"], function (require, exports, assert, async_1, buffer_1, stream_1, utils_1) {
    ...
    suite('Buffer', () => {
        ...
        test('issue #71993 - VSBuffer#toString returns numbers', () => {
            ...
            const buffer = buffer_1.$Fd.wrap(new Uint8Array(data, 3, 2));
            assert.deepStrictEqual(buffer.toString(), 'hi');
        });
        test('bufferToReadable / readableToBuffer', () => {
            const content = 'Hello World';
            const readable = (0, buffer_1.$Qd)(buffer_1.$Fd.fromString(content));
            assert.strictEqual((0, buffer_1.$Pd)(readable).toString(), content);
        });
        test('bufferToStream / streamToBuffer', async () => {
            const content = 'Hello World';
            const stream = (0, buffer_1.$Td)(buffer_1.$Fd.fromString(content));
            assert.strictEqual((await (0, buffer_1.$Rd)(stream)).toString(), content);
        });
        test('bufferedStreamToBuffer', async () => {
            const content = 'Hello World';
            const stream = await (0, stream_1.$yd)((0, buffer_1.$Td)(buffer_1.$Fd.fromString(content)), 1);
            assert.strictEqual((await (0, buffer_1.$Sd)(stream)).toString(), content);
        });
        test('bufferWriteableStream - basics (no error)', async () => {
            ...
            stream.write(buffer_1.$Fd.fromString('Hello'));
            await (0, async_1.$Hg)(0);
            stream.end(buffer_1.$Fd.fromString('World'));
        ...
        test('indexOf', () => {
            const haystack = buffer_1.$Fd.fromString('abcaabbccaaabbbccc');
            assert.strictEqual(haystack.indexOf(buffer_1.$Fd.fromString('')), 0);
            assert.strictEqual(haystack.indexOf(buffer_1.$Fd.fromString('a'.repeat(100))), -1);
            assert.strictEqual(haystack.indexOf(buffer_1.$Fd.fromString('a')), 0);
            assert.strictEqual(haystack.indexOf(buffer_1.$Fd.fromString('c')), 2);
            assert.strictEqual(haystack.indexOf(buffer_1.$Fd.fromString('abcaa')), 0);
            assert.strictEqual(haystack.indexOf(buffer_1.$Fd.fromString('caaab')), 8);
            assert.strictEqual(haystack.indexOf(buffer_1.$Fd.fromString('ccc')), 15);
            assert.strictEqual(haystack.indexOf(buffer_1.$Fd.fromString('cccb')), -1);
        });
        suite('base64', () => {
            /*
            Generated with:
            ...
            */
            ...
            test('encodes', () => {
                for (const [bytes, expected] of testCases) {
                    assert.strictEqual((0, buffer_1.$Zd)(buffer_1.$Fd.wrap(bytes)), expected);
                }
            });
            test('decodes', () => {
                for (const [expected, encoded] of testCases) {
                    assert.deepStrictEqual(new Uint8Array((0, buffer_1.$Yd)(encoded).buffer), expected);
                }
            });
            test('throws error on invalid encoding', () => {
                assert.throws(() => (0, buffer_1.$Yd)('invalid!'));
            });
        });
    });
});
