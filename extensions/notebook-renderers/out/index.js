...
function renderError(outputInfo, outputElement, ctx, trustHtml) {
    ...
    if (err.stack) {
        ...
        const content = (0, textHelper_1.createOutputContent)(outputInfo.id, err.stack ?? '', { linesLimit: ctx.settings.lineLimit, scrollable: outputScrolling, trustHtml });
...
function renderText(outputInfo, outputElement, ctx) {
    ...
    const outputScrolling = scrollingEnabled(outputInfo, ctx.settings);
    const content = (0, textHelper_1.createOutputContent)(outputInfo.id, text, { linesLimit: ctx.settings.lineLimit, scrollable: outputScrolling, trustHtml: false });
