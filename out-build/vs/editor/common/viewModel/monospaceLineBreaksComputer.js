...
define(["require", "exports", "vs/base/common/strings", "vs/editor/common/core/characterClassifier", "vs/editor/common/textModelEvents", "vs/editor/common/modelLineProjectionData"], function (require, exports, strings, characterClassifier_1, textModelEvents_1, modelLineProjectionData_1) {
    ...
    class $rY {
        static create(options) {
            return new $rY(options.get(132 /* EditorOption.wordWrapBreakBeforeCharacters */), options.get(131 /* EditorOption.wordWrapBreakAfterCharacters */));
        }
        constructor(breakBeforeChars, breakAfterChars) {
            this.a = new WrappingCharacterClassifier(breakBeforeChars, breakAfterChars);
        }
                ...
                finalize: () => {
                    ...
                    for (let i = 0, len = requests.length; i < len; i++) {
                        ...
                        if (previousLineBreakData && !previousLineBreakData.injectionOptions && !injectedText) {
                            result[i] = createLineBreaksFromPreviousLineBreaks(this.a, previousLineBreakData, requests[i], tabSize, wrappingColumn, columnsForFullWidthChar, wrappingIndent, wordBreak);
                        }
                        else {
                            result[i] = createLineBreaks(this.a, requests[i], injectedText, tabSize, wrappingColumn, columnsForFullWidthChar, wrappingIndent, wordBreak);
                        }
                    }
                    arrPool1.length = 0;
                    arrPool2.length = 0;
    ...
    class WrappingCharacterClassifier extends characterClassifier_1.$Hs {
            ...
            if (breakOffset === 0) {
                ...
                for (let i = prevBreakOffset - 1; i >= lastBreakingOffset; i--) {
                    ...
                    if (strings.$Re(prevCharCode)) {
                        ...
                        prevCharWidth = (strings.$5e(prevCharCode) ? columnsForFullWidthChar : 1);
                    }
                    if (visibleColumn <= breakingColumn) {
                ...
                if (breakOffset !== 0) {
                    ...
                    if (remainingWidthOfNextLine <= tabSize) {
                        ...
                        let charWidth;
                        if (strings.$Qe(charCodeAtForcedBreakOffset)) {
                            // A surrogate pair must always be considered as a single unit, so it is never to be broken
                            charWidth = 2;
                        }
                        else {
    ...
    function createLineBreaks(classifier, _lineText, injectedTexts, tabSize, firstLineBreakColumn, columnsForFullWidthChar, wrappingIndent, wordBreak) {
        const lineText = textModelEvents_1.$ku.applyInjectedText(_lineText, injectedTexts);
        ...
        if (firstLineBreakColumn === -1) {
            ...
            return new modelLineProjectionData_1.$FU(injectionOffsets, injectionOptions, [lineText.length], [], 0);
        }
        const len = lineText.length;
        if (len <= 1) {
            if (!injectionOptions) {
        ...
        return new modelLineProjectionData_1.$FU(injectionOffsets, injectionOptions, breakingOffsets, breakingOffsetsVisibleColumn, wrappedTextIndentLength);
    }
    ...
    function computeWrappedTextIndentLength(lineText, tabSize, firstLineBreakColumn, columnsForFullWidthChar, wrappingIndent) {
        ...
        if (wrappingIndent !== 0 /* WrappingIndent.None */) {
            const firstNonWhitespaceIndex = strings.$Be(lineText);
