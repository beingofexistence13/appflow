...
define(["require", "exports", "assert", "vs/editor/common/core/wordCharacterClassifier", "vs/editor/common/core/position", "vs/editor/common/core/range", "vs/editor/common/model", "vs/editor/common/model/pieceTreeTextBuffer/pieceTreeTextBufferBuilder", "vs/editor/common/model/pieceTreeTextBuffer/rbTreeBase", "vs/editor/test/common/testTextModel", "vs/base/common/strings", "vs/base/test/common/utils"], function (require, exports, assert, wordCharacterClassifier_1, position_1, range_1, model_1, pieceTreeTextBufferBuilder_1, rbTreeBase_1, testTextModel_1, strings_1, utils_1) {
    ...
    function testLinesContent(str, pieceTable) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            assert.strictEqual(trimLineFeed(pieceTable.getValueInRange(new range_1.$ks(i + 1, 1, i + 1, lines[i].length + (i === lines.length - 1 ? 1 : 2)))), lines[i]);
        }
    }
    function testLineStarts(str, pieceTable) {
        ...
        for (let i = 0; i < lineStarts.length; i++) {
            assert.deepStrictEqual(pieceTable.getPositionAt(lineStarts[i]), new position_1.$js(i + 1, 1));
    ...
    function createTextBuffer(val, normalizeEOL = true) {
        const bufferBuilder = new pieceTreeTextBufferBuilder_1.$tC();
    ...
    function assertTreeInvariants(T) {
        assert(rbTreeBase_1.$$B.color === 0 /* NodeColor.Black */);
        assert(rbTreeBase_1.$$B.parent === rbTreeBase_1.$$B);
        assert(rbTreeBase_1.$$B.left === rbTreeBase_1.$$B);
        assert(rbTreeBase_1.$$B.right === rbTreeBase_1.$$B);
        assert(rbTreeBase_1.$$B.size_left === 0);
        assert(rbTreeBase_1.$$B.lf_left === 0);
        assertValidTree(T);
    }
    function depth(n) {
        if (n === rbTreeBase_1.$$B) {
    ...
    suite('prefix sum for line feed', () => {
        ...
        test('basic', () => {
            ...
            assert.deepStrictEqual(pieceTable.getPositionAt(0), new position_1.$js(1, 1));
            assert.deepStrictEqual(pieceTable.getPositionAt(1), new position_1.$js(1, 2));
            assert.deepStrictEqual(pieceTable.getPositionAt(2), new position_1.$js(2, 1));
            assert.deepStrictEqual(pieceTable.getPositionAt(3), new position_1.$js(2, 2));
            assert.deepStrictEqual(pieceTable.getPositionAt(4), new position_1.$js(3, 1));
            assert.deepStrictEqual(pieceTable.getPositionAt(5), new position_1.$js(3, 2));
            assert.deepStrictEqual(pieceTable.getPositionAt(6), new position_1.$js(4, 1));
        ...
        test('append', () => {
            ...
            assert.deepStrictEqual(pieceTable.getPositionAt(9), new position_1.$js(4, 4));
            assert.strictEqual(pieceTable.getOffsetAt(1, 1), 0);
            assertTreeInvariants(pieceTable);
        });
        test('insert', () => {
            ...
            assert.strictEqual(pieceTable.getLineCount(), 6);
            assert.deepStrictEqual(pieceTable.getPositionAt(6), new position_1.$js(4, 1));
            assert.deepStrictEqual(pieceTable.getPositionAt(7), new position_1.$js(4, 2));
            assert.deepStrictEqual(pieceTable.getPositionAt(8), new position_1.$js(4, 3));
            assert.deepStrictEqual(pieceTable.getPositionAt(9), new position_1.$js(4, 4));
            assert.deepStrictEqual(pieceTable.getPositionAt(12), new position_1.$js(6, 1));
            assert.deepStrictEqual(pieceTable.getPositionAt(13), new position_1.$js(6, 2));
            assert.deepStrictEqual(pieceTable.getPositionAt(14), new position_1.$js(6, 3));
            assert.strictEqual(pieceTable.getOffsetAt(4, 1), 6);
        ...
        test('delete', () => {
            ...
            assert.deepStrictEqual(pieceTable.getPositionAt(7), new position_1.$js(4, 2));
            assert.deepStrictEqual(pieceTable.getPositionAt(8), new position_1.$js(4, 3));
            assert.deepStrictEqual(pieceTable.getPositionAt(9), new position_1.$js(5, 1));
            assert.deepStrictEqual(pieceTable.getPositionAt(11), new position_1.$js(6, 1));
            assert.deepStrictEqual(pieceTable.getPositionAt(12), new position_1.$js(6, 2));
            assert.deepStrictEqual(pieceTable.getPositionAt(13), new position_1.$js(6, 3));
            assert.strictEqual(pieceTable.getOffsetAt(4, 1), 6);
            assert.strictEqual(pieceTable.getOffsetAt(4, 2), 7);
            assert.strictEqual(pieceTable.getOffsetAt(4, 3), 8);
    ...
    suite('get text in range', () => {
        ...
        test('getContentInRange', () => {
            ...
            pieceTable.delete(7, 2);
            // 'a\nb\nc\ndh\ni\njk'
            assert.strictEqual(pieceTable.getValueInRange(new range_1.$ks(1, 1, 1, 3)), 'a\n');
            assert.strictEqual(pieceTable.getValueInRange(new range_1.$ks(2, 1, 2, 3)), 'b\n');
            assert.strictEqual(pieceTable.getValueInRange(new range_1.$ks(3, 1, 3, 3)), 'c\n');
            assert.strictEqual(pieceTable.getValueInRange(new range_1.$ks(4, 1, 4, 4)), 'dh\n');
            assert.strictEqual(pieceTable.getValueInRange(new range_1.$ks(5, 1, 5, 3)), 'i\n');
            assert.strictEqual(pieceTable.getValueInRange(new range_1.$ks(6, 1, 6, 3)), 'jk');
            assertTreeInvariants(pieceTable);
        });
        ...
        test('random test value in range exception', () => {
            ...
            pieceTable.delete(0, 4);
            str = str.substring(0, 0) + str.substring(0 + 4);
            pieceTable.getValueInRange(new range_1.$ks(1, 1, 1, 1));
            assertTreeInvariants(pieceTable);
        });
        test('random tests bug 1', () => {
            let str = '';
    ...
    suite('snapshot', () => {
        ...
        test('immutable snapshot 1', () => {
            const model = (0, testTextModel_1.$O0b)('abc\ndef');
            const snapshot = model.createSnapshot();
            model.applyEdits([
                {
    ...
    suite('chunk based search', () => {
        ...
        test('#45892. For some cases, the buffer is empty but we still try to search', () => {
            ...
            const pieceTable = pieceTree.getPieceTree();
            pieceTable.delete(0, 1);
            const ret = pieceTree.findMatchesLineByLine(new range_1.$ks(1, 1, 1, 1), new model_1.$Eu(/abc/, new wordCharacterClassifier_1.$Js(',./'), 'abc'), true, 1000);
        ...
        test('#45770. FindInNode should not cross node boundary.', () => {
            ...
            pieceTable.delete(16, 1);
            pieceTable.insert(16, ' ');
            const ret = pieceTable.findMatchesLineByLine(new range_1.$ks(1, 1, 4, 13), new model_1.$Eu(/\[/gi, new wordCharacterClassifier_1.$Js(',./'), '['), true, 1000);
            assert.strictEqual(ret.length, 3);
            assert.deepStrictEqual(ret[0].range, new range_1.$ks(2, 3, 2, 4));
            assert.deepStrictEqual(ret[1].range, new range_1.$ks(3, 3, 3, 4));
            assert.deepStrictEqual(ret[2].range, new range_1.$ks(4, 3, 4, 4));
        });
        test('search searching from the middle', () => {
            ...
            const pieceTable = pieceTree.getPieceTree();
            pieceTable.delete(4, 1);
            let ret = pieceTable.findMatchesLineByLine(new range_1.$ks(2, 3, 2, 6), new model_1.$Eu(/a/gi, null, 'a'), true, 1000);
            assert.strictEqual(ret.length, 1);
            assert.deepStrictEqual(ret[0].range, new range_1.$ks(2, 3, 2, 4));
            pieceTable.delete(4, 1);
            ret = pieceTable.findMatchesLineByLine(new range_1.$ks(2, 2, 2, 5), new model_1.$Eu(/a/gi, null, 'a'), true, 1000);
            assert.strictEqual(ret.length, 1);
            assert.deepStrictEqual(ret[0].range, new range_1.$ks(2, 2, 2, 3));
        });
    });
});
