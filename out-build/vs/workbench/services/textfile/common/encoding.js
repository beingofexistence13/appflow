...
define(["require", "exports", "vs/base/common/stream", "vs/base/common/buffer", "vs/amdX", "vs/base/common/cancellation"], function (require, exports, stream_1, buffer_1, amdX_1, cancellation_1) {
    ...
    exports.$rD = exports.$qD = exports.$pD = exports.$oD = exports.$nD = exports.$mD = exports.$lD = exports.$kD = exports.$jD = exports.DecodeStreamErrorKind = exports.$iD = exports.$hD = exports.$gD = exports.$fD = exports.$eD = exports.$dD = exports.$cD = exports.$bD = void 0;
    exports.$bD = 'utf8';
    exports.$cD = 'utf8bom';
    exports.$dD = 'utf16be';
    exports.$eD = 'utf16le';
    function $fD(encoding) {
        return [exports.$bD, exports.$cD, exports.$dD, exports.$eD].some(utfEncoding => utfEncoding === encoding);
    }
    exports.$fD = $fD;
    exports.$gD = [0xFE, 0xFF];
    exports.$hD = [0xFF, 0xFE];
    exports.$iD = [0xEF, 0xBB, 0xBF];
    ...
    class DecoderStream {
        ...
        write(buffer) {
            return this.a.write(buffer);
        }
        end() {
            return this.a.end();
        }
    }
    function $kD(source, options) {
        const minBytesRequiredForDetection = options.minBytesRequiredForDetection ?? options.guessEncoding ? AUTO_ENCODING_GUESS_MIN_BYTES : NO_ENCODING_GUESS_MIN_BYTES;
        return new Promise((resolve, reject) => {
            const target = (0, stream_1.$td)(strings => strings.join(''));
            ...
            const createDecoder = async () => {
                try {
                    ...
                    const detected = await $qD({
                        buffer: buffer_1.$Fd.concat(bufferedChunks),
                    ...
                    if (detected.seemsBinary && options.acceptTextOnly) {
                        throw new $jD('Stream is binary but only text is accepted for decoding', 1 /* DecodeStreamErrorKind.STREAM_IS_BINARY */);
                    }
                    // ensure to respect overwrite of encoding
                    detected.encoding = await options.overwriteEncoding(detected.encoding);
                    // decode and write buffered content
                    decoder = await DecoderStream.create(detected.encoding);
                    const decoded = decoder.write(buffer_1.$Fd.concat(bufferedChunks).buffer);
    ...
    exports.$kD = $kD;
    async function $lD(readable, encoding, options) {
        const iconv = await (0, amdX_1.$aD)('@vscode/iconv-lite-umd', 'lib/iconv-lite-umd.js');
        const encoder = iconv.getEncoder($nD(encoding), options);
                ...
                if (typeof chunk !== 'string') {
                    ...
                    if (!bytesWritten && options?.addBOM) {
                        switch (encoding) {
                            case exports.$bD:
                            case exports.$cD:
                                return buffer_1.$Fd.wrap(Uint8Array.from(exports.$iD));
                            case exports.$dD:
                                return buffer_1.$Fd.wrap(Uint8Array.from(exports.$gD));
                            case exports.$eD:
                                return buffer_1.$Fd.wrap(Uint8Array.from(exports.$hD));
                        }
                    }
                    const leftovers = encoder.end();
                    if (leftovers && leftovers.length > 0) {
                        bytesWritten = true;
                        return buffer_1.$Fd.wrap(leftovers);
                    }
                    return null;
                }
                bytesWritten = true;
                return buffer_1.$Fd.wrap(encoder.write(chunk));
            }
        };
    }
    exports.$lD = $lD;
    async function $mD(encoding) {
        const iconv = await (0, amdX_1.$aD)('@vscode/iconv-lite-umd', 'lib/iconv-lite-umd.js');
        return iconv.encodingExists($nD(encoding));
    }
    exports.$mD = $mD;
    function $nD(enc) {
        if (enc === exports.$cD || enc === null) {
            return exports.$bD; // iconv does not distinguish UTF 8 with or without BOM, so we need to help it
        }
        return enc;
    }
    exports.$nD = $nD;
    function $oD(buffer, bytesRead) {
        if (!buffer || bytesRead < exports.$gD.length) {
            return null;
        }
        const b0 = buffer.readUInt8(0);
        const b1 = buffer.readUInt8(1);
        // UTF-16 BE
        if (b0 === exports.$gD[0] && b1 === exports.$gD[1]) {
            return exports.$dD;
        }
        // UTF-16 LE
        if (b0 === exports.$hD[0] && b1 === exports.$hD[1]) {
            return exports.$eD;
        }
        if (bytesRead < exports.$iD.length) {
            return null;
        }
        const b2 = buffer.readUInt8(2);
        // UTF-8
        if (b0 === exports.$iD[0] && b1 === exports.$iD[1] && b2 === exports.$iD[2]) {
            return exports.$cD;
        }
        return null;
    }
    ...
    async function guessEncodingByBuffer(buffer) {
        const jschardet = await (0, amdX_1.$aD)('jschardet', 'dist/jschardet.min.js');
    ...
    exports.$pD = $pD;
    function $qD({ buffer, bytesRead }, autoGuessEncoding) {
        // Always first check for BOM to find out about encoding
        let encoding = $oD(buffer, bytesRead);
        // Detect 0 bytes to see if file is binary or UTF-16 LE/BE
        // unless we already know that this file has a UTF-16 encoding
        let seemsBinary = false;
        if (encoding !== exports.$dD && encoding !== exports.$eD && buffer) {
