...
define(["require", "exports", "child_process", "fs", "os", "vs/base/common/event", "vs/base/common/path", "vs/base/common/platform", "vs/base/common/ports", "vs/base/node/pfs", "vs/base/node/ports", "vs/platform/files/node/watcher/nodejs/nodejsWatcherLib", "vs/platform/environment/node/argv", "vs/platform/environment/node/argvHelper", "vs/platform/environment/node/stdin", "vs/platform/environment/node/wait", "vs/platform/product/common/product", "vs/base/common/cancellation", "vs/base/common/extpath", "vs/platform/profiling/common/profiling", "vs/base/common/network", "vs/base/common/process", "vs/base/node/unc", "vs/base/common/uri"], function (require, exports, child_process_1, fs_1, os_1, event_1, path_1, platform_1, ports_1, pfs_1, ports_2, nodejsWatcherLib_1, argv_1, argvHelper_1, stdin_1, wait_1, product_1, cancellation_1, extpath_1, profiling_1, network_1, process_1, unc_1, uri_1) {
    ...
    async function main(argv) {
        let args;
        try {
            args = (0, argvHelper_1.$El)(argv);
        }
        catch (err) {
            console.error(err.message);
            return;
        }
        for (const subcommand of argv_1.$xl) {
            if (args[subcommand]) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    if (process.env['VSCODE_DEV']) {
                        tunnelProcess = (0, child_process_1.spawn)('cargo', ['run', '--', subcommand, ...tunnelArgs], { cwd: (0, path_1.$9d)(getAppRoot(), 'cli'), stdio });
                    }
                    else {
                        const appPath = process.platform === 'darwin'
                            // ./Contents/MacOS/Electron => ./Contents/Resources/app/bin/code-tunnel-insiders
                            ? (0, path_1.$9d)((0, path_1.$_d)((0, path_1.$_d)(process.execPath)), 'Resources', 'app')
                            : (0, path_1.$_d)(process.execPath);
                        const tunnelCommand = (0, path_1.$9d)(appPath, 'bin', `${product_1.default.tunnelApplicationName}${platform_1.$i ? '.exe' : ''}`);
        ...
        // Help
        if (args.help) {
            const executable = `${product_1.default.applicationName}${platform_1.$i ? '.exe' : ''}`;
            console.log((0, argv_1.$Bl)(product_1.default.nameLong, executable, product_1.default.version, argv_1.$yl));
        }
        // Version Info
        ...
        // Shell integration
            ...
            console.log((0, path_1.$9d)(getAppRoot(), 'out', 'vs', 'workbench', 'contrib', 'terminal', 'browser', 'media', file));
        }
        ...
        // Write File
            ...
            if (!source || !target || source === target || // make sure source and target are provided and are not the same
                !(0, path_1.$8d)(source) || !(0, path_1.$8d)(target) || // make sure both source and target are absolute paths
        ...
        // Just Code
            ...
            if ((0, stdin_1.$E7b)()) {
                ...
                if (hasReadStdinArg) {
                    stdinFilePath = (0, stdin_1.$G7b)();
                    // returns a file path where stdin input is written into (write in progress).
                    try {
                        await (0, stdin_1.$H7b)(stdinFilePath, !!args.verbose); // throws error if file can not be written
                        // Make sure to open tmp file
                        (0, argvHelper_1.$Fl)(argv, stdinFilePath);
                        // Enable --wait to get all data and ignore adding this to history
                        (0, argvHelper_1.$Fl)(argv, '--wait');
                        (0, argvHelper_1.$Fl)(argv, '--skip-add-to-recently-opened');
                        args.wait = true;
                    ...
                    processCallbacks.push(_ => (0, stdin_1.$F7b)(1000).then(dataReceived => {
                        if (dataReceived) {
                            if (platform_1.$i) {
            ...
            const isMacOSBigSurOrNewer = platform_1.$j && (0, os_1.release)() > '20.0.0';
            ...
            if (args.wait) {
                waitMarkerFilePath = (0, wait_1.$d7b)(args.verbose);
                if (waitMarkerFilePath) {
                    (0, argvHelper_1.$Fl)(argv, '--waitMarkerFilePath', waitMarkerFilePath);
                }
            ...
            if (args['prof-startup']) {
                const portMain = await (0, ports_2.$6f)((0, ports_1.$JS)(), 10, 3000);
                const portRenderer = await (0, ports_2.$6f)(portMain + 1, 10, 3000);
                const portExthost = await (0, ports_2.$6f)(portRenderer + 1, 10, 3000);
                // fail the operation when one of the ports couldn't be acquired.
                if (portMain * portRenderer * portExthost === 0) {
                    throw new Error('Failed to find free ports for profiler. Make sure to shutdown all instances of the editor first.');
                }
                const filenamePrefix = (0, extpath_1.$Qf)((0, os_1.homedir)(), 'prof');
                (0, argvHelper_1.$Fl)(argv, `--inspect-brk=${portMain}`);
                (0, argvHelper_1.$Fl)(argv, `--remote-debugging-port=${portRenderer}`);
                (0, argvHelper_1.$Fl)(argv, `--inspect-brk-extensions=${portExthost}`);
                (0, argvHelper_1.$Fl)(argv, `--prof-startup-prefix`, filenamePrefix);
                (0, argvHelper_1.$Fl)(argv, `--no-cached-data`);
            ...
            if (!isMacOSBigSurOrNewer) {
                ...
                if (args.verbose || args.status) {
                    ...
                    for (const outputType of args.verbose ? ['stdout', 'stderr'] : ['stdout']) {
                        ...
                        const tmpName = (0, extpath_1.$Qf)((0, os_1.tmpdir)(), `code-${outputType}`);
                        ...
                        processCallbacks.push(async (child) => {
                            try {
                                ...
                                await (0, nodejsWatcherLib_1.$Zp)(tmpName, chunk => stream.write(chunk), () => { }, cts.token);
                            }
                            finally {
                ...
                if (env['VSCODE_DEV']) {
                    ...
                    if (launchDirIndex !== -1) {
                        spawnArgs[launchDirIndex] = (0, path_1.$0d)(curdir);
                    }
                }
    ...
    exports.main = main;
    function getAppRoot() {
        return (0, path_1.$_d)(network_1.$2f.asFileUri('').fsPath);
    }
