        ...
        static b(model, offset) {
            ...
            for (let lineNumber = 1; lineNumber <= lineCount; lineNumber++) {
                ...
                if (offsetAfterLine > offset) {
                    return new position_1.$js(lineNumber, offset - offsetBeforeLine + 1);
                }
                offsetBeforeLine = offsetAfterLine;
            }
            return new position_1.$js(lineCount, model.getLineMaxColumn(lineCount));
        }
        static insertSnippet(model, _position) {
            const desiredPosition = model.getValueLengthInRange(new range_1.$ks(1, 1, _position.lineNumber, _position.column));
            ...
            const scanner = (0, json_1.$Jm)(model.getValue());
            let arrayLevel = 0;
            let objLevel = 0;
            ...
            while (scanner.scan() !== 17 /* JSONSyntaxKind.EOF */) {
                ...
                if (currentPos >= desiredPosition && (currentState !== State.INVALID || lastValidPos !== -1)) {
                    ...
                    if (acceptState === State.AFTER_OBJECT) {
                        return {
                            position: this.b(model, acceptPosition),
                            prepend: ',',
                            append: ''
                        };
                    }
            ...
            return {
                position: new position_1.$js(modelLineCount, model.getLineMaxColumn(modelLineCount)),
                prepend: '\n[',
                append: ']'
            };
        }
