...
define(["require", "exports", "child_process", "events", "string_decoder", "vs/base/common/arrays", "vs/base/common/collections", "vs/base/common/glob", "vs/base/common/path", "vs/base/common/strings", "vs/base/common/uri", "vs/workbench/services/search/common/search", "vs/workbench/services/search/common/searchExtTypes", "vscode-regexpp", "@vscode/ripgrep", "./ripgrepSearchUtils"], function (require, exports, cp, events_1, string_decoder_1, arrays_1, collections_1, glob_1, path, strings_1, uri_1, search_1, searchExtTypes_1, vscode_regexpp_1, ripgrep_1, ripgrepSearchUtils_1) {
    ...
    exports.$Fdc = exports.$Edc = exports.$Ddc = exports.$Cdc = exports.$Bdc = exports.$Adc = exports.$zdc = void 0;
    ...
    class $zdc {
        ...
        provideTextSearchResults(query, options, progress, token) {
            ...
            return new Promise((resolve, reject) => {
                ...
                const rgArgs = $Bdc(query, options);
                const cwd = options.folder.fsPath;
                const escapedArgs = rgArgs
                    .map(arg => arg.match(/^-/) ? arg : `'${arg}'`)
                    .join(' ');
                this.a.appendLine(`${rgDiskPath} ${escapedArgs}\n - cwd: ${cwd}`);
                let rgProc = cp.spawn(rgDiskPath, rgArgs, { cwd });
                rgProc.on('error', e => {
                    console.error(e);
                    this.a.appendLine('Error: ' + (e && e.message));
                    reject((0, search_1.$AI)(new search_1.$yI(e && e.message, search_1.SearchErrorCode.rgProcessError)));
                });
                let gotResult = false;
                const ripgrepParser = new $Adc(options.maxResults, cwd, options.previewOptions);
                ...
                rgProc.on('close', () => {
                    ...
                    if (isDone) {
                        ...
                        if (stderr && !gotData && (searchError = rgErrorMsgForDisplay(stderr))) {
                            reject((0, search_1.$AI)(new search_1.$yI(searchError.message, searchError.code)));
                        }
                        else {
                            resolve({ limitHit });
                        }
    ...
    function rgErrorMsgForDisplay(msg) {
        ...
        if (lines.some(l => l.startsWith('regex parse error'))) {
            return new search_1.$yI(buildRegexParseError(lines), search_1.SearchErrorCode.regexParseError);
        }
        const match = firstLine.match(/grep config error: unknown encoding: (.*)/);
        if (match) {
            return new search_1.$yI(`Unknown encoding: ${match[1]}`, search_1.SearchErrorCode.unknownEncoding);
        }
        if (firstLine.startsWith('error parsing glob')) {
            // Uppercase first letter
            return new search_1.$yI(firstLine.charAt(0).toUpperCase() + firstLine.substr(1), search_1.SearchErrorCode.globParseError);
        }
        if (firstLine.startsWith('the literal')) {
            // Uppercase first letter
            return new search_1.$yI(firstLine.charAt(0).toUpperCase() + firstLine.substr(1), search_1.SearchErrorCode.invalidLiteral);
        }
        if (firstLine.startsWith('PCRE2: error compiling pattern')) {
            return new search_1.$yI(firstLine, search_1.SearchErrorCode.regexParseError);
        }
        return undefined;
    }
    ...
    class $Adc extends events_1.EventEmitter {
        constructor(g, h, j) {
            super();
            this.g = g;
            this.h = h;
            ...
            this.c = false;
            this.f = 0;
            this.d = new string_decoder_1.StringDecoder();
        }
        cancel() {
            this.b = true;
        }
        ...
        m(outputLine) {
            ...
            if (parsedLine.type === 'match') {
                ...
                const uri = uri_1.URI.file(path.$9d(this.h, matchPath));
                ...
                const uri = uri_1.URI.file(path.$9d(this.h, contextPath));
        ...
        n(data, uri) {
            ...
            const ranges = (0, arrays_1.$Fb)(data.submatches.map((match, i) => {
                if (this.c) {
                    return null;
                }
                this.f++;
                ...
                return new searchExtTypes_1.$iI(startLineNumber, startCol, endLineNumber, endCol);
            }));
            return (0, ripgrepSearchUtils_1.$sdc)(uri, fullText, ranges, this.j);
        }
        o(data, uri) {
    ...
    function $Bdc(query, options) {
        const args = ['--hidden', '--no-require-git'];
        args.push(query.isCaseSensitive ? '--case-sensitive' : '--ignore-case');
        const { doubleStarIncludes, otherIncludes } = (0, collections_1.$I)(options.includes, (include) => include.startsWith('**') ? 'doubleStarIncludes' : 'otherIncludes');
        ...
        if (query.isMultiline && !query.isRegExp) {
            query.pattern = (0, strings_1.$qe)(query.pattern);
            query.isRegExp = true;
        }
        if (options.usePCRE2) {
            args.push('--pcre2');
        ...
        args.push('--crlf');
        if (query.isRegExp) {
            query.pattern = $Cdc(query.pattern);
            args.push('--engine', 'auto');
        }
        let searchPatternAfterDoubleDashes;
        if (query.isWordMatch) {
            const regexp = (0, strings_1.$ye)(query.pattern, !!query.isRegExp, { wholeWord: query.isWordMatch });
            const regexpStr = regexp.source.replace(/\\\//g, '/'); // RegExp.source arbitrarily returns escaped slashes. Search and destroy.
            args.push('--regexp', regexpStr);
        }
        else if (query.isRegExp) {
            let fixedRegexpQuery = $Ddc(query.pattern);
            fixedRegexpQuery = $Edc(fixedRegexpQuery);
    ...
    function spreadGlobComponents(globComponent) {
        const globComponentWithBraceExpansion = $Fdc(globComponent);
        return globComponentWithBraceExpansion.flatMap((globArg) => {
            const components = (0, glob_1.$pj)(globArg, '/');
            return components.map((_, i) => components.slice(0, i + 1).join('/'));
        });
    }
    function $Cdc(pattern) {
        // Match \u1234
    ...
    function $Fdc(pattern) {
        ...
        let arr = (0, glob_1.$pj)(strInBraces, ',');
        if (!arr.length) {
            // occurs if the braces are empty.
            arr = [''];
        }
        const ends = $Fdc(fixedEnd);
