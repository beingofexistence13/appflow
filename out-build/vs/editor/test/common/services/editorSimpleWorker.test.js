...
define(["require", "exports", "assert", "vs/base/test/common/utils", "vs/editor/common/core/range", "vs/editor/common/services/editorSimpleWorker"], function (require, exports, assert, utils_1, range_1, editorSimpleWorker_1) {
    ...
    suite('EditorSimpleWorker', () => {
        ...
        test('MoreMinimal', () => {
            return worker.computeMoreMinimalEdits(model.uri.toString(), [{ text: 'This is line One', range: new range_1.$ks(1, 1, 1, 17) }], false).then(edits => {
            ...
            const newEdits = await worker.computeMoreMinimalEdits(model.uri.toString(), [
                {
                    ...
                    range: new range_1.$ks(2, 1, 3, 1),
                    text: '',
                }, {
                    range: new range_1.$ks(3, 1, 4, 1),
                    text: '',
                }, {
                    range: new range_1.$ks(4, 2, 4, 3),
                    text: '4',
                }, {
                    range: new range_1.$ks(5, 3, 5, 5),
                    text: '5',
                }
            ], false);
        ...
        test('MoreMinimal, issue #15385 newline changes and other 2/2', function () {
            ...
            return worker.computeMoreMinimalEdits(model.uri.toString(), [{ text: '\n', range: new range_1.$ks(3, 2, 4, 1000) }], false).then(edits => {
                assert.strictEqual(edits.length, 1);
                const [first] = edits;
        ...
        async function testEdits(lines, edits) {
            ...
            return smallerEdits.map(e => ({ range: range_1.$ks.lift(e.range).toString(), text: e.text }));
        }
        ...
        test('computeHumanReadableDiff 2', async () => {
            assert.deepStrictEqual(await testEdits([
                'function test() {}'
                    ...
                    range: new range_1.$ks(1, 1, 1, Number.MAX_SAFE_INTEGER)
    ...
    class PositionOffsetTransformer {
        constructor(d) {
            ...
            this.c.push(d.length + 1);
        }
        getOffset(position) {
            const maxLineOffset = position.lineNumber >= this.c.length ? this.d.length : (this.c[position.lineNumber] - 1);
            return Math.min(this.c[position.lineNumber - 1] + position.column - 1, maxLineOffset);
        }
    }
});
