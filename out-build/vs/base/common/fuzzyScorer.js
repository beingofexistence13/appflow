...
define(["require", "exports", "vs/base/common/comparers", "vs/base/common/filters", "vs/base/common/hash", "vs/base/common/path", "vs/base/common/platform", "vs/base/common/strings"], function (require, exports, comparers_1, filters_1, hash_1, path_1, platform_1, strings_1) {
    ...
    exports.$pq = exports.$oq = exports.$nq = exports.$mq = exports.$lq = exports.$kq = void 0;
    const NO_MATCH = 0;
    const NO_SCORE = [NO_MATCH, []];
    // const DEBUG = true;
    // const DEBUG_MATRIX = false;
    function $kq(target, query, queryLower, allowNonContiguousMatches) {
        if (!target || !query) {
    ...
    function computeCharScore(queryCharAtIndex, queryLowerCharAtIndex, target, targetLower, targetIndex, matchesSequenceLength) {
        ...
        if (targetIndex === 0) {
            ...
            // HTTP => HTTP = not boost
            else if ((0, filters_1.$Bj)(target.charCodeAt(targetIndex)) && matchesSequenceLength === 0) {
                score += 2;
                // if (DEBUG) {
    ...
    function $lq(target, query, patternStart = 0, wordStart = 0) {
    ...
    function doScoreFuzzy2Single(target, query, patternStart, wordStart) {
        const score = (0, filters_1.$Kj)(query.original, query.originalLowercase, patternStart, target, target.toLowerCase(), wordStart, { firstMatchCanBeWeak: true, boostFullMatch: true });
        if (!score) {
            return NO_SCORE2;
        }
        return [score[0], (0, filters_1.$Hj)(score)];
    }
    ...
    function $mq(item, query, allowNonContiguousMatches, accessor, cache) {
    ...
    function doScoreItemFuzzy(label, description, path, query, allowNonContiguousMatches) {
        ...
        if (path && (platform_1.$k ? query.pathNormalized === path : (0, strings_1.$Me)(query.pathNormalized, path))) {
    ...
    function doScoreItemFuzzySingle(label, description, path, query, preferLabelMatches, allowNonContiguousMatches) {
        ...
        if (preferLabelMatches || !description) {
            const [labelScore, labelPositions] = $kq(label, query.normalized, query.normalizedLowercase, allowNonContiguousMatches && !query.expectContiguousMatch);
            if (labelScore) {
                ...
                // beginning.
                const labelPrefixMatch = (0, filters_1.$yj)(query.normalized, label);
                let baseScore;
                if (labelPrefixMatch) {
        ...
        if (description) {
            ...
            const [labelDescriptionScore, labelDescriptionPositions] = $kq(descriptionAndLabel, query.normalized, query.normalizedLowercase, allowNonContiguousMatches && !query.expectContiguousMatch);
    ...
    //#endregion
    //#region Comparers
    function $nq(itemA, itemB, query, allowNonContiguousMatches, accessor, cache) {
        const itemScoreA = $mq(itemA, query, allowNonContiguousMatches, accessor, cache);
        const itemScoreB = $mq(itemB, query, allowNonContiguousMatches, accessor, cache);
    ...
    function fallbackCompare(itemA, itemB, query, accessor) {
        ...
        // compare by label
        if (labelA !== labelB) {
            return (0, comparers_1.$iq)(labelA, labelB, query.normalized);
        }
        // compare by description
        if (descriptionA && descriptionB && descriptionA !== descriptionB) {
            return (0, comparers_1.$iq)(descriptionA, descriptionB, query.normalized);
        }
        // compare by path
        if (pathA && pathB && pathA !== pathB) {
            return (0, comparers_1.$iq)(pathA, pathB, query.normalized);
        }
        // equal
        return 0;
    }
    ...
    function normalizeQuery(original) {
        ...
        const normalized = (0, strings_1.$xe)(pathNormalized).replace(/\s|"/g, '');
        return {
            pathNormalized,
            normalized,
    ...
    function $pq(arg1) {
        if (Array.isArray(arg1)) {
            return $oq(arg1.map(piece => piece.original).join(MULTIPLE_QUERY_VALUES_SEPARATOR));
        }
        return $oq(arg1.original);
    }
    exports.$pq = $pq;
});
//#endregion
