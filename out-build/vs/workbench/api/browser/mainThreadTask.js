...
define(["require", "exports", "vs/nls!vs/workbench/api/browser/mainThreadTask", "vs/base/common/uri", "vs/base/common/uuid", "vs/base/common/types", "vs/base/common/platform", "vs/platform/workspace/common/workspace", "vs/workbench/contrib/tasks/common/tasks", "vs/workbench/contrib/tasks/common/taskService", "vs/workbench/services/extensions/common/extHostCustomers", "vs/workbench/api/common/extHost.protocol", "vs/workbench/services/configurationResolver/common/configurationResolver", "vs/base/common/errors"], function (require, exports, nls, uri_1, uuid_1, Types, Platform, workspace_1, tasks_1, taskService_1, extHostCustomers_1, extHost_protocol_1, configurationResolver_1, errors_1) {
    ...
    (function (ProcessExecutionDTO) {
        ...
        function from(value) {
            const process = Types.$jf(value.name) ? value.name : value.name.value;
            const args = value.args ? value.args.map(value => Types.$jf(value) ? value : value.value) : [];
            const result = {
                process: process,
                args: args
            };
    ...
    (function (ShellExecutionDTO) {
        ...
        function from(value) {
            const result = {};
            if (value.name && Types.$jf(value.name) && (value.args === undefined || value.args === null || value.args.length === 0)) {
    ...
    (function (TaskHandleDTO) {
        function is(value) {
            const candidate = value;
            return candidate && Types.$jf(candidate.id) && !!candidate.workspaceFolder;
        }
        TaskHandleDTO.is = is;
    ...
    (function (TaskDTO) {
        function from(task) {
            if (task === undefined || task === null || (!tasks_1.$eG.is(task) && !tasks_1.$gG.is(task) && !tasks_1.$fG.is(task))) {
                return undefined;
            }
            const result = {
                _id: task._id,
                ...
                execution: undefined,
                presentationOptions: !tasks_1.$fG.is(task) && task.command ? TaskPresentationOptionsDTO.from(task.command.presentation) : undefined,
                isBackground: task.configurationProperties.isBackground,
                problemMatchers: [],
                hasDefinedMatchers: tasks_1.$gG.is(task) ? task.hasDefinedMatchers : false,
            ...
            if (!tasks_1.$fG.is(task) && task.command) {
        ...
        function to(task, workspace, executeOnly, icon, hide) {
            ...
            const result = new tasks_1.$gG(id, // uuidMap.getUUID(identifier)
    ...
    let $psb = class $psb {
        constructor(extHostContext, d, e, f) {
            this.d = d;
            this.e = e;
            this.f = f;
            this.b = extHostContext.getProxy(extHost_protocol_1.$2J.ExtHostTask);
            this.c = new Map();
            this.d.onDidStateChange(async (event) => {
                ...
                if (event.kind === "start" /* TaskEventKind.Start */) {
                    ...
                    if (execution.task?.execution && CustomExecutionDTO.is(execution.task.execution) && event.resolvedVariables) {
                        ...
                        resolvedDefinition = await this.f.resolveAnyAsync(task.getWorkspaceFolder(), execution.task.definition, dictionary);
                    }
                    this.b.$onDidStartTask(execution, event.terminalId, resolvedDefinition);
                }
                else if (event.kind === "processStarted" /* TaskEventKind.ProcessStarted */) {
                    this.b.$onDidStartTaskProcess(TaskProcessStartedDTO.from(task.getTaskExecution(), event.processId));
                }
                else if (event.kind === "processEnded" /* TaskEventKind.ProcessEnded */) {
                    this.b.$onDidEndTaskProcess(TaskProcessEndedDTO.from(task.getTaskExecution(), event.exitCode));
                }
                else if (event.kind === "end" /* TaskEventKind.End */) {
                    this.b.$OnDidEndTask(TaskExecutionDTO.from(task.getTaskExecution()));
                }
            });
        }
        dispose() {
        ...
        $createTaskId(taskDTO) {
            return new Promise((resolve, reject) => {
                const task = TaskDTO.to(taskDTO, this.e, true);
                if (task) {
                    resolve(task._id);
                }
                else {
        ...
        $registerTaskProvider(handle, type) {
            const provider = {
                provideTasks: (validTypes) => {
                    return Promise.resolve(this.b.$provideTasks(handle, validTypes)).then((value) => {
                        const tasks = [];
                        for (const dto of value.tasks) {
                            const task = TaskDTO.to(dto, this.e, true);
                            if (task) {
                                tasks.push(task);
                            }
                            else {
                ...
                resolveTask: (task) => {
                    ...
                    if (dto) {
                        ...
                        return Promise.resolve(this.b.$resolveTask(handle, dto)).then(resolvedTask => {
                            if (resolvedTask) {
                                return TaskDTO.to(resolvedTask, this.e, true, task.configurationProperties.icon, task.configurationProperties.hide);
                            }
                            return undefined;
                        });
                    }
            ...
            const disposable = this.d.registerTaskProvider(provider, type);
            this.c.set(handle, { disposable, provider });
            return Promise.resolve(undefined);
        }
        ...
        $fetchTasks(filter) {
            return this.d.tasks(TaskFilterDTO.to(filter)).then((tasks) => {
                const result = [];
        ...
        async $getTaskExecution(value) {
            if (TaskHandleDTO.is(value)) {
                const workspace = this.g(value.workspaceFolder);
                if (workspace) {
                    const task = await this.d.getTask(workspace, value.id, true);
                    if (task) {
                        return {
                            id: task._id,
                ...
                const task = TaskDTO.to(value, this.e, true);
                return {
                    id: task._id,
                    task: TaskDTO.from(task)
                };
        ...
        $executeTask(value) {
            return new Promise((resolve, reject) => {
                if (TaskHandleDTO.is(value)) {
                    const workspace = this.g(value.workspaceFolder);
                    if (workspace) {
                        this.d.getTask(workspace, value.id, true).then((task) => {
                            if (!task) {
                    ...
                    const task = TaskDTO.to(value, this.e, true);
                    this.d.run(task).then(undefined, reason => {
                        // eat the error, it has already been surfaced to the user and we don't care about it here
                    });
                    const result = {
                        id: task._id,
        ...
        $customExecutionComplete(id, result) {
            return new Promise((resolve, reject) => {
                this.d.getActiveTasks().then((tasks) => {
                    for (const task of tasks) {
                        if (id === task._id) {
                            this.d.extensionCallbackTaskComplete(task, result).then((value) => {
                                resolve(undefined);
                            }, (error) => {
                                reject(error);
                            });
        ...
        $terminateTask(id) {
            return new Promise((resolve, reject) => {
                this.d.getActiveTasks().then((tasks) => {
                    for (const task of tasks) {
                        if (id === task._id) {
                            this.d.terminate(task).then((value) => {
                                resolve(undefined);
                            }, (error) => {
                                reject(undefined);
                            });
                    ...
                    reject(new errors_1.$_('Task to terminate not found'));
                });
            });
        }
        $registerTaskSystem(key, info) {
            ...
            this.d.registerTaskSystem(key, {
                ...
                resolveVariables: (workspaceFolder, toResolve, target) => {
                    ...
                    return Promise.resolve(this.b.$resolveVariables(workspaceFolder.uri, { process: toResolve.process, variables: vars })).then(values => {
                        const partiallyResolvedVars = Array.from(Object.values(values.variables));
                        return new Promise((resolve, reject) => {
                            this.f.resolveWithInteraction(workspaceFolder, partiallyResolvedVars, 'tasks', undefined, target).then(resolvedVars => {
                                if (!resolvedVars) {
                                    resolve(undefined);
                                }
                                const result = {
                ...
                findExecutable: (command, cwd, paths) => {
                    return this.b.$findExecutable(command, cwd, paths);
                }
            });
        }
        async $registerSupportedExecutions(custom, shell, process) {
            return this.d.registerSupportedExecutions(custom, shell, process);
        }
    };
    exports.$psb = $psb;
    exports.$psb = $psb = __decorate([
        (0, extHostCustomers_1.$jbb)(extHost_protocol_1.$1J.MainThreadTask),
        __param(1, taskService_1.$osb),
        __param(2, workspace_1.$Kh),
        __param(3, configurationResolver_1.$NM)
    ], $psb);
});
