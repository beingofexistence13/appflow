...
define(["require", "exports", "assert", "vs/workbench/contrib/terminalContrib/links/browser/terminalLinkHelpers", "vs/base/test/common/utils"], function (require, exports, assert, terminalLinkHelpers_1, utils_1) {
    ...
    suite('Workbench - Terminal Link Helpers', () => {
        ...
        suite('convertLinkRangeToBuffer', () => {
            test('should convert ranges for ascii characters', () => {
                ...
                const bufferRange = (0, terminalLinkHelpers_1.$CWb)(lines, 11, { startColumn: 4, startLineNumber: 1, endColumn: 19, endLineNumber: 1 }, 0);
            ...
            test('should give correct range for links containing multi-character emoji', () => {
                ...
                const bufferRange = (0, terminalLinkHelpers_1.$CWb)(lines, 11, { startColumn: 0 + 1, startLineNumber: 1, endColumn: 2 + 1, endLineNumber: 1 }, 0);
                assert.deepStrictEqual(bufferRange, {
                    start: { x: 1, y: 1 },
                    end: { x: 2, y: 1 }
                });
            ...
            test('should convert ranges for combining characters before the link', () => {
                ...
                const bufferRange = (0, terminalLinkHelpers_1.$CWb)(lines, 11, { startColumn: 4 + 1, startLineNumber: 1, endColumn: 19 + 1, endLineNumber: 1 }, 0);
                assert.deepStrictEqual(bufferRange, {
                    start: { x: 6, y: 1 },
                    end: { x: 9, y: 2 }
                });
            ...
            test('should convert ranges for ascii characters (link starts on wrapped)', () => {
                ...
                const bufferRange = (0, terminalLinkHelpers_1.$CWb)(lines, 11, { startColumn: 15, startLineNumber: 1, endColumn: 30, endLineNumber: 1 }, 0);
                assert.deepStrictEqual(bufferRange, {
                    start: { x: 4, y: 2 },
                    end: { x: 7, y: 3 }
                });
            ...
            test('regression test #147619: 获取模板 25235168 的预览图失败', () => {
                ...
                assert.deepStrictEqual((0, terminalLinkHelpers_1.$CWb)(lines, 30, {
                    startColumn: 1,
                    startLineNumber: 1,
                    endColumn: 5,
                    endLineNumber: 1
            ...
            test('should convert ranges for several wide characters before and inside the link', () => {
                ...
                const bufferRange = (0, terminalLinkHelpers_1.$CWb)(lines, 11, { startColumn: 14, startLineNumber: 1, endColumn: 31, endLineNumber: 1 }, 0);
    ...
    class TestBufferLine {
        constructor(a, length, isWrapped) {
            this.a = a;
            this.length = length;
        ...
        getCell(x) {
            ...
            for (let i = 0; i <= x - wideNullCellOffset + emojiOffset; i++) {
                let char = this.a.charAt(i);
                if (char === '\ud83d') {
                    // Make "🙂"
                    char += '\ude42';
                }
                cells.push(char);
                if (this.a.charAt(i) === TEST_WIDE_CHAR || char.charCodeAt(0) > 255) {
